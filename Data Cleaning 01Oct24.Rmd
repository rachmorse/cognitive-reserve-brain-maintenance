...
waha_df_struct <- merge(waha_df_struct_controls, waha_df_struct, by = c("id", "timepoint", "sex", "age", "education",
               "MMSE", "HIP_left","HIP_right", "HIP_total", "ICV"), all = TRUE) 
...
columns_to_keep <- c("id", "timepoint", 
                     "sex", "age", "edu",
...
Before getting into the FC data, this code is to generate the regions in the Shirer atlas that are also in the task invariant network
...
Now clean the functional WAHA data and merge it with the structural
...
# Reshape data for timepoint 1
waha_baseline <- waha_df %>%
  filter(timepoint == 1) %>%
  rename_with(~ paste0(.x, ".1"), -c(id, timepoint, sex, edu, mmse))

# Reshape data for timepoint 2
waha_followup <- waha_df %>%
  filter(timepoint == 2) %>%
  rename_with(~ paste0(.x, ".2"), -c(id, timepoint, sex, edu, mmse))

# Reshape data for timepoint 3
waha_followup2 <- waha_df %>%
  filter(timepoint == 3) %>%
  rename_with(~ paste0(.x, ".3"), -c(id, timepoint, sex, edu, mmse))

# Join the reshaped data frames together
waha_df <- waha_baseline %>%
  full_join(waha_followup, by = c("id", "sex", "edu", "mmse")) %>%
  full_join(waha_followup2, by = c("id", "sex", "edu", "mmse"))
...
# Reformat the df with one row for each participant
# Reshape data for timepoint 1
cobra_baseline <- cobra_df %>%
  filter(timepoint == 1) %>%
  rename_with(~ paste0(.x, ".1"), -c(id, sex, edu, mmse))

# Reshape data for timepoint 2
cobra_followup <- cobra_df %>%
  filter(timepoint == 2) %>%
  rename_with(~ paste0(.x, ".2"), -c(id, sex, edu, mmse))

# Join the reshaped data frames together
cobra_df <- full_join(cobra_baseline, cobra_followup, by = c("id", "sex", "edu", "mmse"))

# Remove timepoint variable
cobra_df <- cobra_df %>%
  select(-starts_with("timepoint"))
...
# Reshape data for timepoint 1
betula_baseline <- betula_df %>%
  filter(timepoint == 1) %>%
  rename_with(~ paste0(.x, ".1"), -c(id, sex, edu, mmse))

# Reshape data for timepoint 2
betula_followup <- betula_df %>%
  filter(timepoint == 2) %>%
  rename_with(~ paste0(.x, ".2"), -c(id, timepoint, sex, edu, mmse))

# Join the reshaped data frames together
betula_df <- betula_baseline %>%
  full_join(betula_followup, by = c("id", "sex", "edu", "mmse")) 
...
oslo_struct <- oslo_struct %>%
  rename(timepoint = "SubjectRound_id",
    id = "Subject_id",
    age = "Age",
    sex = "Sex",
...
Readin Oslo functional data and merge dfs
...
columns_to_keep_scanner <- c("timepoint", 
                     "id",
                     "scanner"
)
...
# Reformat the df with one row for each participant
# Reshape data for timepoint 1
oslo_baseline <- oslo_df %>%
  filter(timepoint == 1) %>%
  rename_with(~ paste0(.x, ".1"), -c(id, timepoint, sex, edu, mmse))

# Reshape data for timepoint 2
oslo_followup <- oslo_df %>%
  filter(timepoint == 2) %>%
  rename_with(~ paste0(.x, ".2"), -c(id, timepoint, sex, edu, mmse))

# Reshape data for timepoint 3
oslo_followup2 <- oslo_df %>%
  filter(timepoint == 3) %>%
  rename_with(~ paste0(.x, ".3"), -c(id, timepoint, sex, edu, mmse))

# Reshape data for timepoint 4
oslo_followup3 <- oslo_df %>%
  filter(timepoint == 4) %>%
  rename_with(~ paste0(.x, ".4"), -c(id, timepoint, sex, edu, mmse))

# Reshape data for timepoint 5
oslo_followup4 <- oslo_df %>%
  filter(timepoint == 5) %>%
  rename_with(~ paste0(.x, ".5"), -c(id, timepoint, sex, edu, mmse))

# Reshape data for timepoint 6
oslo_followup5 <- oslo_df %>%
  filter(timepoint == 6) %>%
  rename_with(~ paste0(.x, ".6"), -c(id, timepoint, sex, edu, mmse))

# Join the reshaped data frames together
oslo_df <- oslo_baseline %>%
  full_join(oslo_followup, by = c("id", "sex", "edu", "mmse")) %>%
  full_join(oslo_followup2, by = c("id", "sex", "edu", "mmse")) %>%
  full_join(oslo_followup3, by = c("id", "sex", "edu", "mmse")) %>%
  full_join(oslo_followup4, by = c("id", "sex", "edu", "mmse")) %>%
  full_join(oslo_followup5, by = c("id", "sex", "edu", "mmse"))

oslo_df <- oslo_df %>%
  select(-starts_with("timepoint"))

# Make a df to check that the scanners are being selected correctly
oslo_df_scanners <- select(oslo_df, 
...
exclude_cols <- c("id", "sex", "mmse", "edu", "study", 
                  "age_baseline",
