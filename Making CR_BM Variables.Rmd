...
data <- data %>%
  mutate(
    hippocampus_slopes_scaled = hippocampus_slopes / max(
      abs(min(hippocampus_slopes, na.rm = TRUE)),
      abs(max(hippocampus_slopes, na.rm = TRUE))
    ),
    memory_slopes_scaled = memory_slopes / max(
      abs(min(memory_slopes, na.rm = TRUE)),
      abs(max(memory_slopes, na.rm = TRUE))
    )
  )
```
