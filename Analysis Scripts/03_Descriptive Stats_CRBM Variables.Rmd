...
```{r}
# First create a scaled and centered version of the HC and memory annual change variables using the residuals
emhc_data <- emhc_data %>%
  mutate(
    hippocampus_slopes_centered = res_hc_slopes - mean(res_hc_slopes, na.rm = TRUE),
    memory_slopes_centered = res_mem_slopes - mean(res_mem_slopes, na.rm = TRUE),
    res_hc_slopes_scaled =
      hippocampus_slopes_centered / max(
        abs(min(hippocampus_slopes_centered, na.rm = TRUE)),
        abs(max(hippocampus_slopes_centered, na.rm = TRUE))
      ),
    res_mem_slopes_scaled =
      memory_slopes_centered / max(
