...
```{r}
# Switch variable to chr
dfs <- list(waha_df, cobra_df, betula_df, oslo_df)
cols_to_fix <- c("id", "sex", "study", "cohort")
dfs_fixed <- map(dfs, ~ .x %>%
  mutate(across(all_of(intersect(cols_to_fix, names(.x))), as.character)))
waha_df <- dfs_fixed[[1]]
cobra_df <- dfs_fixed[[2]]
betula_df <- dfs_fixed[[3]]
oslo_df <- dfs_fixed[[4]]
```
...
calculate_slope <- function(df, hip_total_cols, icv_cols) {
  ...
  for (study in unique(df$study)) {
    for (i in seq_along(hip_total_cols)) {
      ...
      # Skip if there are no data for this study/timepoint
...
slopes_wide <- slopes %>%
  pivot_wider(
    names_from = timepoint, values_from = slope, 
    names_prefix = "slope."
  )
...
# Check the output data to make sure it makes sense
correlation <- cor(merged_cohorts$HIP_total.2, merged_cohorts$adj_HIP_total.2, use = "complete.obs")
print(correlation)

ggplot(merged_cohorts, aes(x = HIP_total.2, y = adj_HIP_total.2)) +
  geom_point()
```
...
process_data <- function(data, n_timepoints, mem_vars) {
  ...
  for (i in 1:n_timepoints) {
    for (mem_var in mem_vars) {
      ...
      if (col_name %in% names(data)) {
        ...
        data[[paste0("sc_", col_name)]] <- as.numeric(scaled_values)
      } else {
  ...
  for (i in 1:nrow(data)) {
    ...
    if (sum(!is.na(em_data)) > 1 & sum(!is.na(hc_data)) > 1) {
      ...
      data[i, "mem_time"] <- max(age[!is.na(em_data)]) - min(age[!is.na(em_data)])
...
oslo_df <- merged_cohorts %>%
  filter(study %in% c("oslo_skyra", "oslo_avanto"))
...
Now create functional connectivity obs and time variables
```{r}
...
# Uncomment this code to evaluate and check GAM assumptions

# # Loop through k values and check results for mem
# out_mem <- data.frame(k = integer(), AIC = double(), BIC = double(), edf = double())
# for (kk in 1:27) {
#   k_use <- kk + 3
#   model_mem_gam <- gam(memory_slopes ~ s(age_followup, k = k_use),
#                       data = clean_data)
#   edf_val <- as.numeric(summary(model_mem_gam)$s.table[,"edf"][1])
#   out_mem <- rbind(out_mem, data.frame(k = k_use,
#                                AIC = AIC(model_mem_gam),
#                                BIC = BIC(model_mem_gam),
#                                edf = edf_val))
# }
# 
# # Loop through k values and check results for hc
# out_hc <- data.frame(k = integer(), AIC = double(), BIC = double(), edf = double())
# for (kk in 1:27) {
#   k_use <- kk + 3
#   model_hc_gam <- gam(hc_slopes ~ s(age_followup, k = k_use),
#                       data = clean_data)
#   edf_val <- as.numeric(summary(model_hc_gam)$s.table[,"edf"][1])
#   out_hc <- rbind(out_hc, data.frame(k = k_use,
#                                AIC = AIC(model_hc_gam),
#                                BIC = BIC(model_hc_gam),
#                                edf = edf_val))
# }

# Check the HC model summaries and comparisons
m_lin_hc <- gam(hc_slopes ~ age_followup, data = clean_data)
m_smooth_hc <- gam(hc_slopes ~ s(age_followup, k = 3), data = clean_data)
gam.check(m_smooth_hc)

anova(m_lin_hc, m_smooth_hc, test = "Chisq")

# Check the mem models
m_lin_mem <- gam(memory_slopes ~ age_followup, data = clean_data)
m_smooth_mem <- gam(memory_slopes ~ s(age_followup, k = 12), data = clean_data)
gam.check(m_smooth_mem)

anova(m_lin_mem, m_smooth_mem, test = "Chisq")

# Compare the two models
cor(resid(m_lin_mem), resid(m_smooth_mem))
cor(resid(m_lin_hc), resid(m_smooth_hc))
```

```{r}
# In order to be able to save these models, the NAs need to be removed
...
mem_res_data <- clean_data %>%
  ...
  mutate(
    res_mem_slopes_age = residuals(
      gam(memory_slopes ~ s(age_followup, k = 12), data = .)
    )
  ) %>%
...
hc_res_data <- clean_data %>%
  ...
  mutate(
    res_hc_slopes_age = residuals(
      gam(hc_slopes ~ s(age_followup, k = 3), data = .)
    )
  ) %>%
...
clean_data <- merge(clean_data, emhc_data, by = c("id"), all = TRUE)
```

Now write the data frames as CSVs
```{r}
write.csv(clean_data, file = "/tsd/p274/data/durable/projects/p027-cr_bm/Clean Data All Cohorts/Wide Format/Analysed data.csv")
```
