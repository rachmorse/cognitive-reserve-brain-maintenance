...
Remove anyone outside of 3SD for these new variables by cohrot
...
replace_outliers_with_na <- function(x, k = 3){
  mean <- mean(x, na.rm = TRUE)
  sd <- sd(x, na.rm = TRUE)
  x[abs(x - mean) > k * sd] <- NA
  x
}

# Applying the function
merged_cohorts <- merged_cohorts %>%
  group_by(cohort) %>%                     # compute 3SD within each cohort
  mutate(across(all_of(vars_to_clean), ~ replace_outliers_with_na(.x, k = 3))) %>%
  ungroup()
```
...
create_bl_fu_columns <- function(data) {
  ...
  data <- data %>%
    mutate(across(ends_with("_baseline") | ends_with("_followup"),
                ~ as.numeric(.)))
  return(data)
}
...
mem_res_data <- clean_data %>%
  ...
  mutate(
    res_mem_slopes_age = residuals(
      lm(memory_slopes ~ age_baseline, data = .)
    )
  ) %>%
...
hc_res_data <- clean_data %>%
  ...
  mutate(
    res_hc_slopes_age = residuals(
      lm(hc_slopes ~ age_baseline, data = .)
    )
  ) %>%
