...
# for (k in 1:27) {
#   model_mem_gam <- gam(memory_slopes ~ s(age_followup, k = k+3), data = clean_data)
#
#   cat("Model with k =", k + 3, "\n")
#   cat("AIC:", AIC(model_mem_gam), "\n")
#   cat("BIC:", BIC(model_mem_gam), "\n")
#   cat("\n")
# }
#
# # Loop through k values and print results
# for (k in 1:27) {
#   model_hc_gam <- gam(hc_slopes ~ s(age_followup, k = k+3), data = clean_data)
#
#   cat("Model with k =", k + 3, "\n")
#   cat("AIC:", AIC(model_hc_gam), "\n")
#   cat("BIC:", BIC(model_hc_gam), "\n")
#   cat("\n")
# }
#
# gam.check(model_mem_gam)
# gam.check(model_hc_gam)
# 
# # Now check that the gam fits better than the linear model 
# m_lin_hc    <- gam(hc_slopes ~  age_followup,            data = clean_data, method = "ML")
# m_smooth_hc <- gam(hc_slopes ~ s(age_followup, k = 4),   data = clean_data, method = "ML")
# anova(m_lin_hc, m_smooth_hc, test = "Chisq")   
# 
# m_lin_mem    <- gam(memory_slopes ~  age_followup,            data = clean_data, method = "ML")
# m_smooth_mem <- gam(memory_slopes ~ s(age_followup, k = 25),   data = clean_data, method = "ML")
...
mem_res_data <- clean_data %>%
  ...
  mutate(
    res_mem_slopes_age = residuals(
      gam(memory_slopes ~ s(age_followup, k = 25), data = .)
    )
  ) %>%
...
hc_res_data <- clean_data %>%
  ...
  mutate(
    res_hc_slopes_age = residuals(
      gam(hc_slopes ~ s(age_followup, k = 4), data = .)
    )
  ) %>%
