...
edu_mem_long <- get_regression_stats(scale(memory_slopes) ~ scale(edu) + study + age_baseline + sex, data = clean_data, vars)
edu_cr_long <- get_regression_stats(scale(memory_slopes) ~ scale(edu) * scale(hc_slopes) + study + age_baseline + sex, data = clean_data, vars)
...
Longitudinal repeated-measures analyses provided no evidence that education moderated the relationship between hippocampal volume and memory (_p_ = `r format_p_value(edu_cr_int$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_cr_int", "adjusted_p_value"])`). Similarly, the longitudinal change-based analyses also found no evidence that education moderated the hippocampal-memory change relationship (_p_ = `r format_p_value(edu_cr_long$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_cr_long", "adjusted_p_value"])`). There was no evidence of a significant relationship between our CR variable and education in a repeated-measures analysis (_p_ = `r format_p_value(edu_cr$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_cr", "adjusted_p_value"])`). Nonetheless, education was significantly correlated with memory in the repeated-measures analysis (β = `r round(edu_mem$coef, 4)`, 95% CI `r round(edu_mem$ci[1], 4)` to `r round(edu_mem$ci[2], 4)`, _p_ = `r format(format_p_value(edu_mem$p_value), scientific = FALSE)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_mem", "adjusted_p_value"])`, marginal r² = `r round(edu_mem$marginal_r2, 4)`, conditional r² = `r round(edu_mem$conditional_r2, 4)`; Table 5); however, in the change-based analysis, education was not associated with memory change (_p_ = `r format_p_value(edu_mem_long$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_mem_long", "adjusted_p_value"])`)
...
cr_dmn_stats_long <- get_regression_stats(scale(memory_slopes) ~ scale(av_dmn) * scale(hc_slopes) + study + age_baseline + sex + edu, data = clean_data, priority_var_cr)
cr_ecn_stats_long <- get_regression_stats(scale(memory_slopes) ~ scale(av_ec) * scale(hc_slopes) + study + age_baseline + sex + edu, data = clean_data, priority_var_cr)
cr_sn_stats_long <- get_regression_stats(scale(memory_slopes) ~ scale(av_sn) * scale(hc_slopes) + study + age_baseline + sex + edu, data = clean_data, priority_var_cr)
```
...
long_fc <- data.frame(
  ...
  "**Education * Adjusted Hippocampal Volume**" = c(
    ...
    "**Education**", "_Coefficient (95% CI)_",
    paste0(round(edu_cr$coef, 4), " (", round(edu_cr$ci[1], 4), " to ", format(round(edu_cr$ci[2], 4), scientific = FALSE), ")")
  ),
  " " = c(
    ...
    "", "_Adjusted p-value_", format_p_value(p_value_df[p_value_df$model_name == "edu_cr", "adjusted_p_value"])
    ),
  " " = c(
    "_Marginal R²_", round(edu_cr_int$marginal_r2, 4),
    "", "_Marginal R²_", round(edu_mem$marginal_r2, 4),
    "", "_Adjusted R²_", round(edu_cr$adj_r2, 4)
  ),
  ...
  "**ECN Connectivity * Adjusted Hippocampal Volume**" = c(
    ...
    "**ECN Connectivity**", "_Coefficient (95% CI)_",
    paste0(round(cr_ec$coef, 4), " (", round(cr_ec$ci[1], 4), " to ", round(cr_ec$ci[2], 4), ")")
  ),
  " " = c(
  ...
  "**Anterior SN Connectivity * Adjusted Hippocampal Volume**" = c(
    ...
    "**Anterior SN Connectivity**", "_Coefficient (95% CI)_",
    paste0(round(cr_sn$coef, 4), " (", round(cr_sn$ci[1], 4), " to ", round(cr_sn$ci[2], 4), ")")
  ),
  " " = c(
