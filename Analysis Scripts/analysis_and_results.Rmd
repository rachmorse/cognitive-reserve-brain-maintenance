...
__Figure 2:__ _Memory and Hippocampal Annual Change_
...
__Figure 3:__ _Brain Maintenance and Cognitive Reserve_

```{r fig2, echo=FALSE, dpi = 400, fig.width=7, fig.height=4, message=FALSE, warning=FALSE}
combined_crbm
```
<small>A) The association between hippocampus and memory annual change, adjusted for age. The color represents the brain maintenance value for a given participant. B) The same association between hippocampus and memory annual change, adjusted for age, with the color representing the cognitive reserve value for a given participant.</small>
...
__Figure 4:__ _The Memory-Hippocampal Change Relationship Across Levels of Cognitive Reserve_


The relationship between hippocampal and memory change, both adjusted for age, shown as a function of CR. Data are divided into quantile bins of CR, and each line segment displays a linear fit between hippocampal and memory change for each bin. Line color indicates the mean CR value for each bin. Solid line segments indicate the portion of each fit that falls within the observed data range for that CR bin, while semi-transparent segments represent extrapolated portions outside the observed data range. The figure demonstrates that the slope of the hippocampus change-memory change relationship becomes progressively flatter with increasing CR, indicating that individuals with higher CR exhibit weaker coupling between hippocampal and memory change.
...
edu_cr <- get_regression_stats(scale(edu) ~ scale(CR) + study + age_baseline + sex + mem_time, data = clean_data, int_vars)
```

__**Cognitive Reserve, Education, and Functional Connectivity:**__

Longitudinal analyses provided no evidence that education moderated the relationship between hippocampal volume and memory (_p_ = `r format_p_value(edu_cr_int$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_cr_int", "adjusted_p_value"])`) or the hippocampus change-memory change relationship (_p_ = `r format_p_value(edu_cr_long$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_cr_long", "adjusted_p_value"])`). We also found no significant relationship between our CR variable and education (_p_ = `r format_p_value(edu_cr$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_cr", "adjusted_p_value"])`). Education was, however, significantly correlated with memory (β = `r round(edu_mem$coef, 4)`, 95% CI `r round(edu_mem$ci[1], 4)` to `r round(edu_mem$ci[2], 4)`, _p_ = `r format(format_p_value(edu_mem$p_value), scientific = FALSE)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_mem", "adjusted_p_value"])`, marginal r² = `r round(edu_mem$marginal_r2, 4)`, conditional r² = `r round(edu_mem$conditional_r2, 4)`) but not with memory change (_p_ = `r format_p_value(edu_mem_long$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_mem_long", "adjusted_p_value"])`; Table 5)
...
cr_fc
```
The left panel depicts a Johnson-Neyman plot of how the hippocampal volume-memory relationship changes as anterior Salience Network connectivity changes. The plot shows that as anterior Salience Network connectivity decreases, the hippocampal volume-memory relationship increases, only becoming significant when Salience Network connectivity is less than -0.39SD from the mean. Above that threshold, the hippocampal volume-memory relationship is not significant. The right panel shows the hippocampal volume-memory relationship changing as Default Mode Network connectivity changes. The plot shows that as Default Mode Network connectivity decreases, the hippocampal volume-memory relationship increases. However, this hippocampal-memory relationship is significant for both high and low Default Mode Network connectivity values, only becoming non-significant when Default Mode Network connectivity is between -0.24 and +3.54 SD from the mean. Within that range, the hippocampal volume-memory relationship is not significant. Apparent values beyond three SD appear as we examined variables across cohorts after within-cohort outlier exclusion. The lower panel highlights the anterior Salience Network as defined in the Shirer atlas on the left and the Default Mode Network on the right. Abbreviations: SD standard deviation, n.s. not significant.
...
__Supplementary Figure 2__: _Memory and Hippocampal Volume Change with Age_
```{r echo=FALSE, message=FALSE, warning=FALSE}
# Define colors
