...
Longitudinal mixed-effects analyses provided no evidence that education moderated the relationship between hippocampal volume and memory (_p_ = `r format_p_value(edu_cr_int$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_cr_int", "adjusted_p_value"])`), nor was there a significant relationship between our CR variable and education (_p_ = `r format_p_value(edu_cr$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_cr", "adjusted_p_value"])`) in high CR participants. However, education was significantly correlated with memory (β = `r round(edu_mem$coef, 4)`, 95% CI `r round(edu_mem$ci[1], 4)` to `r round(edu_mem$ci[2], 4)`, _p_ = `r format(format_p_value(edu_mem$p_value), scientific = FALSE)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_mem", "adjusted_p_value"])`, marginal r² = `r round(edu_mem$marginal_r2, 4)`, conditional r² = `r round(edu_mem$conditional_r2, 4)`), and the correlation remained significant when hippocampal change was included in the model (β = `r round(edu_cr_hc$coef, 4)`, 95% CI `r round(edu_cr_hc$ci[1], 4)` to `r round(edu_cr_hc$ci[2], 4)`, _p_ = `r format(format_p_value(edu_cr_hc$p_value), scientific = FALSE)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_cr_hc", "adjusted_p_value"])`, marginal r² = `r round(edu_cr_hc$marginal_r2, 4)`, conditional r² = `r round(edu_cr_hc$conditional_r2, 4)`). 
...
cr_fc
```
The top panel depicts a Johnson-Neyman plot of how the hippocampal volume-memory relationship changes as anterior Salience Network connectivity changes. The plot shows that as anterior Salience Network connectivity decreases, the hippocampal volume-memory relationship increases, only becoming significant when Salience Network connectivity is less than -0.02SD above the mean. Above that threshold, the hippocampal volume–memory relationship is not significant. The bottom panel shows the hippocampal volume-memory relationship this time depicted with Default Mode Network connectivity changes. The plot shows that as Default Mode Network connectivity decreases, the hippocampal volume-memory relationship increases. However, this hippocampal-memory relationship is significant for both high and low Default Mode Network connectivity values, only becoming non-significant when Default Mode Network connectivity is between 0.08-3.70SD above the mean. Within that range, the hippocampal volume–memory relationship is not significant. Abbreviations: SN Salience Network, DMN Default Mode Network, n.s. not significant. 
...
# for (k in 1:27) {
#   model_mem <- gamm4(memory ~ s(age, k = k+3), data = long_clean_data, random = ~(1|id), REML   = FALSE)
# 
...
# # Loop through k values and print results
# for (k in 1:27) {
#   model_hc <- gamm4(adj_HIP_total ~ s(age, k = k+3), data = long_clean_data, random = ~(1|id), REML   = FALSE)
# 
#   cat("Model with k =", k + 3, "\n")
...
# }
# 
# gamm_memory <- gamm4(memory ~ s(age, k = 4), data = long_clean_data, random = ~(1|id), REML   = FALSE)
# gam.check(gamm_memory$gam)
# 
# gamm_hc <- gamm4(adj_HIP_total ~ s(age, k = 5), data = long_clean_data, random = ~(1|id), REML   = FALSE)
