...
Additionally, we found longitudinal evidence that as age increased both memory and hippocampus volume decreased, though we observed interindividual differences in these relationships (Supplementary Figure 2). Both the LME model and GAMM fit to this repeated measures data showed evidence of significant relationships between memory and hippocampal volume regressed on age; however, the non-linear GAMMs provided better overall fits (Supplementary Table 1). We include further details in the Supplementary Materials.
...
rg_mem_hc <- ref_grid(
  ...
  at = list(hc_slopes = seq(
    from = min(plot_data$hc_slopes),
    to = max(plot_data$hc_slopes),
    length.out = 50
  )),
)
...
We established CR and BM variables as illustrated in Figure 3, which plots each participant’s hippocampal and memory annual change - both adjusted for age - together with their corresponding CR or BM value. Because these trajectories are adjusted for age, individuals whose hippocampus atrophies and memory declines less than expected for their age appear to show increases in both hippocampal volume and memory; however, these positive values reflect better-than-expected stability for the participants’ age. This adjustment for age allows the CR and BM variables to consider the faster rate of decline in hippocampal volume and memory observed among our older participants (Supplementary Figure 2).
...
combined_crbm
```
<small>A) The association between hippocampus and memory annual change, adjusted for age. The color represents the BM value for a given participant. B) The same association between hippocampus and memory annual change, adjusted for age, with the color representing the CR value for a given participant. Abbreviations: CR Cognitive Reserve, BM Brain Maintenance.</small>

The mean BM value was  `r round(mean_bm, 4)` (range = `r round(min_bm, 4)` - `r round(max_bm, 4)`), and the mean CR value was `r round(mean_cr, 4)` (range = `r round(min_cr, 4)` - `r round(max_cr, 4)`). We classified participants into high and low groups for each variable using a median split. Of these, `r cr_bm_high`/`r total_participants` participants showed both high CR and BM, whereas `r cr_bm_low`/`r total_participants` participants were low in both. To visualize how CR moderates the hippocampus change-memory change relationship, we examined the association between hippocampal and memory change across increasing levels of CR (Figure 4). The slope of the hippocampus change-memory change relationship decreased as CR increased, indicating that individuals with higher CR showed weaker coupling between hippocampal decline and memory decline. We found evidence of a weak association between our CR and BM variables (Spearman’s ρ = `r round(crbm_corr$estimate, 4)`, _p_ = `r format(format_p_value(crbm_corr$p.value), scientific = FALSE)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "crbm_corr", "adjusted_p_value"])`). Sensitivity analyses showed that alternative formulations of the CR and BM variables were highly correlated with the main definitions (Spearman’s ρ = 0.74-1.00, _ps_ < 0.0001; Supplementary Table 2, Supplementary Figure 3). We discuss these alternative formulations in greater detail in the Supplementary Materials. 

 
__Figure 4:__ _Difference in the Memory-Hippocampal Change Relationship Across Levels of Cognitive Reserve_


The relationship between hippocampal and memory change, both adjusted for age, shown as a function of CR. Data are divided into quantile bins of CR, and each line segment displays a linear fit between hippocampal and memory change for each bin. Line color indicates the mean CR value for each bin. Solid line segments indicate the portion of each fit that falls within the observed data range for that CR bin, while faint segments represent extrapolated portions outside the observed data range. The figure demonstrates that the slope of the hippocampus change-memory change relationship becomes progressively flatter with increasing CR, indicating that individuals with higher CR exhibit weaker coupling between hippocampal and memory change.
...
# Check if edu is a cross-sectional CR factor
...
# Then edu and memory level
edu_mem <- get_mixed_effects_stats(scale(memory) ~ scale(edu) + scale(time) + (1|id) + (1|study) + age + sex, data = long_clean_data, vars)

# Then longitudinal CR and memory change
edu_mem_long <- get_regression_stats(scale(memory_slopes) ~ scale(edu) + study + age_baseline + mem_time + sex, data = clean_data, vars)
edu_cr_long <- get_regression_stats(scale(memory_slopes) ~ scale(edu) * scale(hc_slopes) + mem_time + study + age_baseline + sex, data = clean_data, vars)

# Then is edu related to our CR variable
edu_cr <- get_regression_stats(scale(edu) ~ scale(CR) + study + age_baseline + sex, data = clean_data, int_vars)
```

__**Cognitive Reserve, Education, and Functional Connectivity:**__

Longitudinal repeated measures analyses provided no evidence that education moderated the relationship between hippocampal volume and memory (_p_ = `r format_p_value(edu_cr_int$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_cr_int", "adjusted_p_value"])`). Similarly, the longitudinal change-based analyses also found no evidence that education moderated the hippocampus change-memory change relationship (_p_ = `r format_p_value(edu_cr_long$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_cr_long", "adjusted_p_value"])`). Moreover, in a repeated measures analysis, we did not find a significant relationship between our CR variable and education (_p_ = `r format_p_value(edu_cr$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_cr", "adjusted_p_value"])`). Nonetheless, education was significantly correlated with memory in a repeated measures analysis (β = `r round(edu_mem$coef, 4)`, 95% CI `r round(edu_mem$ci[1], 4)` to `r round(edu_mem$ci[2], 4)`, _p_ = `r format(format_p_value(edu_mem$p_value), scientific = FALSE)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_mem", "adjusted_p_value"])`, marginal r² = `r round(edu_mem$marginal_r2, 4)`, conditional r² = `r round(edu_mem$conditional_r2, 4)`); however, in the change-based analysis, education was not associated with memory change (_p_ = `r format_p_value(edu_mem_long$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_mem_long", "adjusted_p_value"])`; Table 5)

```{r echo=FALSE, message=FALSE, warning=FALSE}
# Before running the average connectivity models, calculate average FC
clean_data <- clean_data %>%
  mutate(av_sn = rowMeans(across(c(paste0("sn.", 1:6))), na.rm = TRUE)) %>% 
  mutate(av_dmn = rowMeans(across(c(paste0("dmn.", 1:6))), na.rm = TRUE)) %>% 
  mutate(av_ec = rowMeans(across(c(paste0("ec.", 1:6))), na.rm = TRUE))  
...
priority_var_cr <- c(
  ...
  "scale(av_dmn):scale(hc_slopes)",
  "scale(av_ec):scale(hc_slopes)",
  "scale(av_sn):scale(hc_slopes)"
)

# CR on mem-hc relationship
...
# CR on mem-hc change relationship
cr_dmn_stats_long <- get_regression_stats(scale(memory_slopes) ~ scale(av_dmn) * scale(hc_slopes) + study + age_baseline + mem_time + sex + edu, data = clean_data, priority_var_cr)
cr_ecn_stats_long <- get_regression_stats(scale(memory_slopes) ~ scale(av_ec) * scale(hc_slopes) + study + age_baseline + mem_time + sex + edu, data = clean_data, priority_var_cr)
cr_sn_stats_long <- get_regression_stats(scale(memory_slopes) ~ scale(av_sn) * scale(hc_slopes) + study + age_baseline + mem_time + sex + edu, data = clean_data, priority_var_cr)
```
...
# Relation to our CR var
cr_dmn <- get_mixed_effects_stats(scale(dmn) ~ scale(CR) + (1|id) + (1|study) + age + time + sex + edu, data = long_clean_data, priority_var_int_cr)
cr_ec <- get_mixed_effects_stats(scale(ec) ~ scale(CR) + (1|id) + (1|study) + age + time + sex + edu, data = long_clean_data, priority_var_int_cr)
cr_sn <- get_mixed_effects_stats(scale(sn) ~ scale(CR) + (1|id) + (1|study) + age + time + sex + edu, data = long_clean_data, priority_var_int_cr)

# Relation to mem level
...
# Relation to mem change
cr_dmn_mem_long <- get_regression_stats(scale(memory_slopes) ~ scale(av_dmn) + study + age_baseline + mem_time + sex + edu, data = clean_data, priority_var_int_cr)
cr_ec_mem_long <- get_regression_stats(scale(memory_slopes) ~ scale(av_ec) + study + age_baseline + mem_time + sex + edu, data = clean_data, priority_var_int_cr)
cr_sn_mem_long <- get_regression_stats(scale(memory_slopes) ~ scale(av_sn) + study + age_baseline + mem_time + sex + edu, data = clean_data, priority_var_int_cr)
```

With functional connectivity, the longitudinal repeated measures analyses revealed that anterior SN connectivity moderated the relationship between hippocampal volume and memory (β = `r round(cr_sn_stats$coef, 4)`, 95% CI `r round(cr_sn_stats$ci[1], 4)` to `r round(cr_sn_stats$ci[2], 4)`, _p_ = `r format(format_p_value(cr_sn_stats$p_value), scientific = FALSE)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_sn_stats", "adjusted_p_value"])`, marginal r² = `r round(cr_sn_stats$marginal_r2, 4)`, conditional r² = `r round(cr_sn_stats$conditional_r2, 4)`), such that participants with higher anterior SN connectivity did not have a significant relationship between memory and hippocampal volume. This moderation analysis is summarized in Figure 5 which depicts how the hippocampal volume-memory relationship increases as anterior SN connectivity decreases, demonstrating a significant positive relationship between memory and hippocampal volume only for participants with lower anterior SN connectivity. Anterior SN connectivity was also positively correlated with memory in the repeated measures model (β = `r round(cr_sn_mem$coef, 4)`, 95% CI `r round(cr_sn_mem$ci[1], 4)` to `r round(cr_sn_mem$ci[2], 4)`, _p_ = `r format(format_p_value(cr_sn_mem$p_value), scientific = FALSE)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_sn_mem", "adjusted_p_value"])`, marginal r² = `r round(cr_sn_mem$marginal_r2, 4)`, conditional r² = `r round(cr_sn_mem$conditional_r2, 4)`). However, in the longitudinal change-based analyses, average anterior SN connectivity did not moderate the hippocampus change-memory change relationship (_p_ = `r format(format_p_value(cr_sn_stats_long$p_value), scientific = FALSE)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_sn_stats_long", "adjusted_p_value"])`), nor did it correlate with memory change (_p_ = `r format(format_p_value(cr_sn_mem_long$p_value), scientific = FALSE)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_sn_mem_long", "adjusted_p_value"])`). Lastly, there was no evidence of a relationship between our CR variable and anterior SN connectivity (_p_ = `r format(format_p_value(cr_sn$p_value), scientific = FALSE)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_sn", "adjusted_p_value"])`).

With the DMN, there was evidence in the longitudinal repeated measures analysis that DMN connectivity moderated the hippocampal volume-memory relationship (β = `r round(cr_dmn_stats$coef, 4)`, 95% CI `r round(cr_dmn_stats$ci[1], 4)` to `r round(cr_dmn_stats$ci[2], 4)`, _p_ = `r format_p_value(cr_dmn_stats$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_dmn_stats", "adjusted_p_value"])`, marginal r² = `r round(cr_dmn_stats$marginal_r2, 4)`, conditional r² = `r round(cr_dmn_stats$conditional_r2, 4)`); however, unlike with anterior SN connectivity, when DMN connectivity was high, there was a significant relationship between hippocampal volume and memory, such that decreases in hippocampal volume were associated with increases in memory. Figure 5 depicts how the hippocampal volume-memory relationship increases as DMN connectivity decreases, showing a significant positive relationship between memory and hippocampal volume for participants with lower DMN connectivity and for participants with higher DMN connectivity. This indicates that, among participants with higher DMN connectivity, there is a negative hippocampal-memory relationship. Additionally, in the repeated measures analysis, there was no evidence of a relationship between DMN connectivity and memory (_p_ = `r format_p_value(cr_dmn_mem$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_dmn_mem", "adjusted_p_value"])`). In the longitudinal change-based analysis, we did not find evidence that average DMN connectivity moderated the hippocampus change-memory change relationship (_p_ = `r format_p_value(cr_dmn_stats_long$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_dmn_stats_long", "adjusted_p_value"])`), nor that it was related to memory change (_p_ = `r format_p_value(cr_dmn_mem_long$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_dmn_mem_long", "adjusted_p_value"])`). Finally, we did not find evidence of a relationship between our CR variable and DMN connectivity (_p_ = `r format_p_value(cr_dmn$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_dmn", "adjusted_p_value"])`).

With ECN connectivity, in a longitudinal repeated measures analysis, there was no evidence that ECN connectivity moderated the relationship between hippocampal volume and memory (_p_ = `r format_p_value(cr_ecn_stats$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_ecn_stats", "adjusted_p_value"])`), or of a relationship between ECN connectivity and memory (_p_ = `r format_p_value(cr_ec_mem$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_ec_mem", "adjusted_p_value"])`). In the longitudinal change-based analysis, we also found no evidence of a moderation from average ECN connectivity on the hippocampus change-memory change relationship (_p_ = `r format_p_value(cr_ecn_stats_long$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_ecn_stats_long", "adjusted_p_value"])`), or of a relationship between average ECN connectivity and memory change (_p_ = `r format_p_value(cr_ec_mem_long$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_ec_mem_long", "adjusted_p_value"])`). Lastly, we found no evidence of a relationship between our CR variable and ECN connectivity (_p_ = `r format_p_value(cr_ec$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_ec", "adjusted_p_value"])`). The functional connectivity results are summarized in Table 5. 

```{r echo=FALSE, message=FALSE, warning=FALSE}
# Fit the model 
model_sn <- lmer(scale(memory) ~ scale(sn) * scale(adj_HIP_total) + (1|id) + 
              (1|study) + age + time + sex + edu, 
              data = long_clean_data, REML = FALSE)

# Johnson-Neyman plot
jn_result_sn <- johnson_neyman(model_sn, 
                           pred = "scale(adj_HIP_total)", 
                           modx = "scale(sn)",
                           plot = TRUE)

jn_plot_sn <- jn_result_sn$plot  
jn_plot_sn <- jn_plot_sn + 
...
jn_plot_sn <- jn_plot_sn +
  coord_cartesian(xlim = c(-2.6, 6.5), ylim = c(-1.1, 0.6))
...
# Fit 2nd model 
model_dmn <- lmer(scale(memory) ~ scale(dmn) * scale(adj_HIP_total) + (1|id) + 
              (1|study) + age + time + sex + edu, 
              data = long_clean_data, REML = FALSE)

# Johnson-Neyman plot
jn_result_dmn <- johnson_neyman(model_dmn, 
                           pred = "scale(adj_HIP_total)", 
                           modx = "scale(dmn)",
                           plot = TRUE)

jn_plot_dmn <- jn_result_dmn$plot +
  xlab("Default Mode Network Connectivity") +
  ylab("Effect of Hippocampal Volume on Memory") +
  labs(title = NULL) +
  theme(
    legend.position = "bottom",
    plot.tag.position = c(0.05, 0.05),  # bottom-left
    plot.tag = element_text(size = 12, face = "bold"),
    plot.caption = element_text(size = 10, hjust = -0.2),
    axis.title.x = element_text(size = 11),
    axis.title.y = element_text(size = 11),
    plot.title = element_text(face = "plain", size = 11)
  )

# Remove the significance shading bar manually
jn_plot_dmn <- jn_plot_dmn + theme(legend.position = "none")

jn_plot_dmn <- jn_plot_dmn +
  guides(fill = "none", alpha = "none", colour = "none", linetype = "none")

jn_plot_dmn <- jn_plot_dmn +
  coord_cartesian(xlim = c(-2.6, 6.5), ylim = c(-1.1, 0.6))

cr_fc_dmn <- jn_plot_dmn + 
  xlab("Default Mode Network Connectivity") +
  ylab(" ") 

# Combine the plots and collect legend
cr_fc <- cr_fc_sn + cr_fc_dmn +
  plot_layout(ncol = 2, guides = "collect") &
  theme(legend.position = "bottom")
```

__Figure 5:__ _Anterior Salience Network and Default Mode Network Functional Connectivity and the Hippocampal-Memory Relationship_

```{r fig5, echo=FALSE, dpi = 400, fig.width=6, fig.height=4, message=FALSE, warning=FALSE}
cr_fc
```
The left panel depicts a Johnson-Neyman plot of how the hippocampal volume-memory relationship changes as anterior Salience Network connectivity changes. The plot shows that as anterior Salience Network connectivity decreases, the hippocampal volume-memory relationship increases, only becoming significant when Salience Network connectivity is less than 0.39SD below the mean. Above that threshold, the hippocampal volume-memory relationship is not significant. The right panel shows the hippocampal volume-memory relationship this time depicted with Default Mode Network connectivity changes. The plot shows that as Default Mode Network connectivity decreases, the hippocampal volume-memory relationship increases. However, this hippocampal-memory relationship is significant for both high and low Default Mode Network connectivity values, only becoming non-significant when Default Mode Network connectivity is between -0.24-3.54SD above the mean. Within that range, the hippocampal volume-memory relationship is not significant. Apparent values beyond three SD appear as we examined variables across cohorts after within-cohort outlier exclusion. The lower panel highlights the anterior Salience Network as defined in the Shirer atlas on the left and the Default Mode Network on the right. Abbreviations: SD standard deviation, n.s. not significant.
...
long_fc <- data.frame(
  ...
  "**Education * Adjusted Hippocampal Volume**" = c(
    "_Coefficient (95% CI)_",
    paste0(round(edu_cr_int$coef, 4), " (", round(edu_cr_int$ci[1], 4), " to ", round(edu_cr_int$ci[2], 4), ")"),
    "**Education**", "_Coefficient (95% CI)_",
    paste0(round(edu_mem$coef, 4), " (", round(edu_mem$ci[1], 4), " to ", round(edu_mem$ci[2], 4), ")"),
    "**Education * Adjusted Hippocampal Change**", "_Coefficient (95% CI)_",
    paste0(round(edu_cr_long$coef, 4), " (", round(edu_cr_long$ci[1], 4), " to ", format(round(edu_cr_long$ci[2], 4), scientific = FALSE), ")"),
    "**Education**", "_Coefficient (95% CI)_",
    paste0(round(edu_mem_long$coef, 4), " (", round(edu_mem_long$ci[1], 4), " to ", round(edu_mem_long$ci[2], 4), ")")
  ),
  " " = c(
    "_Adjusted p-value_", format_p_value(p_value_df[p_value_df$model_name == "edu_cr_int", "adjusted_p_value"]),
    "", "_Adjusted p-value_", format_p_value(p_value_df[p_value_df$model_name == "edu_mem", "adjusted_p_value"]),
    "", "_Adjusted p-value_", format_p_value(p_value_df[p_value_df$model_name == "edu_cr_long", "adjusted_p_value"]),
    "", "_Adjusted p-value_", format_p_value(p_value_df[p_value_df$model_name == "edu_mem_long", "adjusted_p_value"])
    ),
  " " = c(
    "_Marginal R²_", round(edu_cr_int$marginal_r2, 4),
    "", "_Marginal R²_", round(edu_mem$marginal_r2, 4),
    "", "_Adjusted R²_", round(edu_cr_long$adj_r2, 4),
    "", "_Adjusted R²_", round(edu_mem_long$adj_r2, 4)
  ),
  "**DMN Connectivity * Adjusted Hippocampal Volume**" = c(
    "_Coefficient (95% CI)_",
    ...
    "**DMN Connectivity * Adjusted Hippocampal Change**", "_Coefficient (95% CI)_",
    paste0(round(cr_dmn_stats_long$coef, 4), " (", round(cr_dmn_stats_long$ci[1], 4), " to ", round(cr_dmn_stats_long$ci[2], 4), ")"),
    "**DMN Connectivity**", "_Coefficient (95% CI)_",
    paste0(round(cr_dmn_mem_long$coef, 4), " (", round(cr_dmn_mem_long$ci[1], 4), " to ", round(cr_dmn_mem_long$ci[2], 4), ")")
  ),
  " " = c(
    ...
    "", "_Adjusted p-value_", format_p_value(p_value_df[p_value_df$model_name == "cr_dmn_stats_long", "adjusted_p_value"]),
    "", "_Adjusted p-value_", format_p_value(p_value_df[p_value_df$model_name == "cr_dmn_mem_long", "adjusted_p_value"])
  ),
  " " = c(
    "_Marginal R²_", round(cr_dmn_stats$marginal_r2, 4),
    "", "_Marginal R²_", round(cr_dmn_mem$marginal_r2, 4),
    "", "_Adjusted R²_", round(cr_dmn_stats_long$adj_r2, 4),
    "", "_Adjusted R²_", round(cr_dmn_mem_long$adj_r2, 4)
  ),
  "**ECN Connectivity * Adjusted Hippocampal Volume**" = c(
    "_Coefficient (95% CI)_",
    ...
    "**ECN Connectivity * Adjusted Hippocampal Change**", "_Coefficient (95% CI)_",
    paste0(round(cr_ecn_stats_long$coef, 4), " (", round(cr_ecn_stats_long$ci[1], 4), " to ", round(cr_ecn_stats_long$ci[2], 4), ")"),
    "**ECN Connectivity**", "_Coefficient (95% CI)_",
    paste0(round(cr_ec_mem_long$coef, 4), " (", round(cr_ec_mem_long$ci[1], 4), " to ", round(cr_ec_mem_long$ci[2], 4), ")")
  ),
  " " = c(
    ...
    "", "_Adjusted p-value_", format_p_value(p_value_df[p_value_df$model_name == "cr_ecn_stats_long", "adjusted_p_value"]),
    "", "_Adjusted p-value_", format_p_value(p_value_df[p_value_df$model_name == "cr_ec_mem_long", "adjusted_p_value"])
  ),
  " " = c(
    "_Marginal R²_", round(cr_ecn_stats$marginal_r2, 4),
    "", "_Marginal R²_", round(cr_ec_mem$marginal_r2, 4),
    "", "_Adjusted R²_", round(cr_ecn_stats_long$adj_r2, 4),
    "", "_Adjusted R²_", round(cr_ec_mem_long$adj_r2, 4)
  ),
  "**Anterior SN Connectivity * Adjusted Hippocampal Volume**" = c(
    "_Coefficient (95% CI)_",
    ...
    "**Anterior SN Connectivity * Adjusted Hippocampal Change**", "_Coefficient (95% CI)_",
    paste0(round(cr_sn_stats_long$coef, 4), " (", round(cr_sn_stats_long$ci[1], 4), " to ", round(cr_sn_stats_long$ci[2], 4), ")"),
    "**Anterior SN Connectivity**", "_Coefficient (95% CI)_",
    paste0(round(cr_sn_mem_long$coef, 4), " (", round(cr_sn_mem_long$ci[1], 4), " to ", round(cr_sn_mem_long$ci[2], 4), ")")
  ),
  " " = c(
    ...
    "", "_Adjusted p-value_", format_p_value(p_value_df[p_value_df$model_name == "cr_sn_stats_long", "adjusted_p_value"]),
    "", "_Adjusted p-value_", format_p_value(p_value_df[p_value_df$model_name == "cr_sn_mem_long", "adjusted_p_value"])
  ),
  " " = c(
    "_Marginal R²_", round(cr_sn_stats$marginal_r2, 4),
    "", "_Marginal R²_", round(cr_sn_mem$marginal_r2, 4),
    "", "_Adjusted R²_", round(cr_sn_stats_long$adj_r2, 4),
    "", "_Adjusted R²_", round(cr_sn_mem_long$adj_r2, 4)
  )
)
```

__Table 5__: _Models Examining Education, Functional Connectivity and Cognitive Reserve_
```{r tab5, echo=FALSE} 
# Format and print the table
kable(long_fc, format = "markdown", col.names = c(" ", "**Education * Adjusted Hippocampal Volume**", " ", " ", "**DMN Connectivity * Adjusted Hippocampal Volume**", " ", " ", "**ECN Connectivity * Adjusted Hippocampal Volume**", " ", " ", "**Anterior SN Connectivity * Adjusted Hippocampal Volume**", " ", " "))
```
...
Supplementary Figure 1 depicts distributions of normalized memory composite scores by cohort across timepoints. The box-and-jitter plots show the distribution of normalized memory scores including the mean, standard deviation, and number of participants by cohort following the harmonization process. The resulting composites overlap across cohorts, with similar means and distributions across timepoints, indicating that the harmonization procedure places the cohorts’ episodic memory composites on a comparable scale.
...
# for (k in 1:27) {
#   model_mem <- gamm4(memory ~ s(age, k = k+3, bs = "cs") + time + sex + edu, data = long_clean_data, random = ~(1|id) + (1|study), REML = FALSE)
# 
...
# }
# 
# gamm_memory <- gamm4(memory ~ s(age, k = 4, bs = "cs") + time + sex + edu, data = long_clean_data, random = ~(1|id) + (1|study), REML = FALSE)
...
get_gamm_metrics <- function(k, df, response_var, lme_mod) {
  if (k > 1) {
    ...
    mod <- gamm4(as.formula(paste(response_var, "~ 1 + s(age, k =", k, ", bs = 'cs') + time + sex + edu")),
      data = df, 
      random = ~ (1|id) + (1|study), 
      REML   = FALSE
    )
    ...
    return(data.frame(
      k = k, AIC = aic, BIC = bic, LogLik = loglik,
...
gamm_compare <- function(response_var, mod_lme) {
  compare_gamm <- get_gamm_metrics(k, long_clean_data, response_var, mod_lme)
  return(compare_gamm)
}
...
gamm_table <- data.frame(
  ...
  " " = c(
    "_LRT p-value_",
    format_p_value(gamm_compare_em_memory %>% select(p_value_lrt)),
    " "
  ),
  ...
  " " = c(
    "_LRT p-value_",
    format_p_value(gamm_compare_em_adj_hc %>% select(p_value_lrt)),
    " "
  )
)
```

We first compared Generalized Additive Mixed Models (GAMM) and Linear Mixed Effects (LME) examining the relationship between age and repeated measures of memory and hippocampal volume. We conducted checks of the effective degrees of freedom (EDF), the Akaike Information Criterion (AIC), the Bayesian Information Criterion (BIC), and the p-value to determine the adequate number of basis dimensions in models. The age-term was significant in the memory GAMM (EDF = `r round(gamm_compare_em_memory$EDF, 4)`, _p_-value = `r format_p_value(gamm_compare_em_memory$p_value_gamm)`) and the hippocampus GAMM (EDF = `r round(gamm_compare_em_adj_hc$EDF, 4)`, _p_-value = `r format_p_value(gamm_compare_em_adj_hc$p_value_gamm)`). For the LME models, both models had significant age coefficients (memory: β = `r round(mem_age_lme$coef, 4)`, 95% CI `r round(mem_age_lme$ci[1], 4)` to `r round(mem_age_lme$ci[2], 4)`, _p_ = `r format(format_p_value(mem_age_lme$p_value), scientific = FALSE)`; hippocampal volume β = `r round(hc_age_lme$coef, 4)`, 95% CI `r round(hc_age_lme$ci[1], 4)` to `r round(hc_age_lme$ci[2], 4)`, _p_ = `r format(format_p_value(hc_age_lme$p_value), scientific = FALSE)`). For both memory and hippocampal volume, the GAMM fits were better according to AIC, BIC, and the Likelihood Ratio Test (LRT).
...
mem_age <- ggplot(long_clean_data, aes(x = age, y = memory, color = Study, group = id)) +
  ...
  labs(x = "Age", y = "Episodic Memory", color = "Study", tag = "A)") +
  theme_minimal() +
  theme(legend.position = "bottom",
...
hc_age <- ggplot(long_clean_data, aes(x = age, y = adj_HIP_total, color = Study, group = id)) +
  ...
  geom_line(data = hc_plot, aes(x = age, y = fit),
            inherit.aes = FALSE, color = "grey", linewidth = 1) +
  labs(x = "Age", y = "Hippocampal Volume", color = "Study", tag = "B)") +
  theme_minimal() +
...
# Memory level and CR variable LME
mem_cr_lme <- list(edu_mem, cr_dmn_mem, cr_ec_mem, cr_sn_mem)

# Memory change and CR variable
mem_cr_long <- list(edu_mem_long, cr_dmn_mem_long, cr_ec_mem_long, cr_sn_mem_long)

# Moderation mem-hc LME
mem_hc_lme <- list(edu_cr_int, cr_dmn_stats, cr_ecn_stats, cr_sn_stats)

# Moderation mem-hc change 
mem_hc_long <- list(edu_cr_long, cr_dmn_stats_long, cr_ecn_stats_long, cr_sn_stats_long)

# Our CR variable relation
cr_var <- list(edu_cr, cr_dmn, cr_ec, cr_sn)
...
mem_cr_lme_p_values <- adjust_p_values(
  mem_cr_lme,
  c("edu_mem", "cr_dmn_mem", "cr_ec_mem", "cr_sn_mem")
)
mem_cr_long_p_values <- adjust_p_values(
  mem_cr_long,
  c("edu_mem_long", "cr_dmn_mem_long", "cr_ec_mem_long", "cr_sn_mem_long")
)
mem_hc_lme_p_values <- adjust_p_values(
  mem_hc_lme,
  c("edu_cr_int", "cr_dmn_stats", "cr_ecn_stats", "cr_sn_stats")
)
mem_hc_long_p_values <- adjust_p_values(
  mem_hc_long,
  c("edu_cr_long", "cr_dmn_stats_long", "cr_ecn_stats_long", "cr_sn_stats_long")
)
cr_var_p_values <- adjust_p_values(
  cr_var,
  c("edu_cr", "cr_dmn", "cr_ec", "cr_sn")
)
...
p_value_df <- rbind(
  ...
  mem_cr_lme_p_values, 
  mem_cr_long_p_values, 
  mem_hc_lme_p_values,
  mem_hc_long_p_values,
  cr_var_p_values
)
