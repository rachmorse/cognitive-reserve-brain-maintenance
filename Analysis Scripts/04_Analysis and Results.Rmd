...
index_plot <- ggplot(clean_data, aes(x = total_cr_or_bm, y = 0, fill = closer_to)) +
  ...
  theme(
    ...
    plot.margin = margin(t = 0.35, r = 0.35, b = 0.35, l = 0, unit = "cm"),
    plot.title = element_text(hjust = 0.5)
  )
```
...
# And for BM
clean_data$reverse_bm_group <- cut(clean_data$reverse_bm,
  breaks = quantile(clean_data$reverse_bm, probs = seq(0, 1, (1 / 2)), na.rm = TRUE),
  include.lowest = TRUE,
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# Remove instances of NA
long_data_vis_res <- long_data_vis %>%
  filter(!is.na(age) & !is.na(memory))

# Fit the mixed-effects model
model_res_lme <- lmer(memory ~ age * reverse_cr_group + (1 | id), data = long_data_vis)

# Define a range of ages for prediction
age_seq <- seq(min(long_data_vis_res$age), max(long_data_vis_res$age), length.out = 100)
...
predictions <- do.call(rbind, lapply(unique(long_data_vis_res$reverse_cr_group), function(res) {
  new_data <- data.frame(
    age = age_seq,
...
# Create the plot
cr_age_plot <- ggplot() +
  geom_line(data = long_data_vis_res, aes(x = age, y = memory, group = id), color = "lightgray", alpha = 0.5) + # Plot individual trajectories
  geom_point(data = long_data_vis_res, aes(x = age, y = memory), color = "gray", size = 1) + # Add scatter points
  ...
  labs(x = "Age", y = "Memory", color = "Group", fill = "Group") +
  theme_minimal() +
  theme(legend.position = "bottom")
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# Fit the mixed-effects model
model_res_lme <- lmer(memory ~ age * reverse_bm_group + (1 | id), data = long_data_vis)

# Define a range of ages for prediction
age_seq <- seq(min(long_data_vis_res$age), max(long_data_vis_res$age), length.out = 100)

# Generate predictions for each BM group
predictions <- do.call(rbind, lapply(unique(long_data_vis_res$reverse_bm_group), function(res) {
  new_data <- data.frame(
    age = age_seq,
