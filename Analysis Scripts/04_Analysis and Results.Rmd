...
# Remove unneeded columns before creating long df
clean_data <- clean_data %>% 
  select(-X, -X.1)
...
clean_data <- clean_data %>%
  rowwise() %>%
  mutate(time = mean(c(mem_time, hc_time, fc_time), na.rm = TRUE)) %>%
  ungroup()
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
long_clean_data_edu_mem_hc <- long_clean_data %>%
  filter(!is.na(memory) & !is.na(edu) & !is.na(hc_slopes))
...
edu_cr_int <- get_mixed_effects_stats(scale(memory) ~ scale(edu) * scale(adj_HIP_total) + (1|id) + study + age_baseline + mem_time + sex, data = long_clean_data_edu_mem_hc, vars)

# Then is edu related to our CR variable
edu_cr <- get_regression_stats(scale(edu) ~ scale(reverse_cr) + study + age_baseline + mem_time + sex, data = clean_data, int_vars)
```

__**Cognitive Reserve Associations with Education and Functional Connectivity:**__

There was no evidence that education is a CR measure in our data (_p_ = `r format_p_value(edu_cr_int$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_cr_int", "adjusted_p_value"])`, nor was there a significant relationship between our CR variable (_p_ = `r format_p_value(edu_cr$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_cr", "adjusted_p_value"])`) and education (Table 5). 

```{r echo=FALSE, message=FALSE, warning=FALSE}
# Create a CR & education table
edu_table_data <- data.frame(
  " " = c(
    "", "**Education * Adjusted Hippocampal Volume**"
  ),
  "**Memory**" = c(
    "_Coefficient (95% CI)_",
  ...
  " " = c(
    "_Marginal R²_",
    round(edu_cr_int$marginal_r2, 3)
  ),
  " " = c(
    "",
    "**Cognitive Reserve**"
...
__Table 5__: _Regressions with Cognitive Reserve and Education_

```{r tab6, echo=FALSE} 
# Format and print the table
kable(edu_table_data, format = "markdown", col.names = c(" ", "**Memory**", " ", " ", " ", "**Education**", " ", " "))
```
...
long_clean_data_hc_mem_fc <- long_clean_data %>%
  filter(!is.na(adj_HIP_total) & !is.na(memory) & !is.na(sex) & !is.na(edu) & !is.na(sn))
...
cr_dmn_stats <- get_mixed_effects_stats(scale(memory) ~ scale(dmn) * scale(adj_HIP_total) + (1|id) + study + age + mem_time + sex + edu, data = long_clean_data_hc_mem_fc, priority_var_cr)
cr_ecn_stats <- get_mixed_effects_stats(scale(memory) ~ scale(ec) * scale(adj_HIP_total) + (1|id) + study + age + mem_time + sex + edu, data = long_clean_data_hc_mem_fc, priority_var_cr)
cr_sn_stats <- get_mixed_effects_stats(scale(memory) ~ scale(sn) * scale(adj_HIP_total) + (1|id) + study + age + mem_time + sex + edu, data = long_clean_data_hc_mem_fc, priority_var_cr)

# Cross-sectional analyses
cr_dmn_stats_bl <- get_regression_stats(scale(memory_baseline) ~ scale(dmn_baseline) * scale(adj_HIP_total_baseline) + study + age_baseline + sex + edu, data = clean_data, priority_var_cr)
cr_dmn_stats_fu <- get_regression_stats(scale(memory_followup) ~ scale(dmn_followup) * scale(adj_HIP_total_followup) + study + age_followup + sex + edu, data = clean_data, priority_var_cr)

cr_ecn_stats_bl <- get_regression_stats(scale(memory_baseline) ~ scale(ec_baseline) * scale(adj_HIP_total_baseline) + study + age_baseline + sex + edu, data = clean_data, priority_var_cr)
cr_ecn_stats_fu <- get_regression_stats(scale(memory_followup) ~ scale(ec_followup) * scale(adj_HIP_total_followup) + study + age_followup + sex + edu, data = clean_data, priority_var_cr)

cr_sn_stats_bl <- get_regression_stats(scale(memory_baseline) ~ scale(sn_baseline) * scale(adj_HIP_total_baseline) + study + age_baseline + sex + edu, data = clean_data, priority_var_cr)
cr_sn_stats_fu <- get_regression_stats(scale(memory_followup) ~ scale(sn_followup) * scale(adj_HIP_total_followup) + study + age_followup + sex + edu, data = clean_data, priority_var_cr)
```
...
# Filter to those with the right data
long_clean_data_fc <- long_clean_data %>%
  filter(!is.na(sn_slopes))

# Filter to those aging CR-like
long_cr_pathway_fc <- long_clean_data_fc %>%
  filter(pathway == "CR Pathway") 

cr_dmn <- get_mixed_effects_stats(scale(dmn) ~ scale(reverse_cr) + (1|id) + study, data = long_cr_pathway_fc, priority_var_int_cr)
cr_ec <- get_mixed_effects_stats(scale(ec) ~ scale(reverse_cr) + (1|id) + study, data = long_cr_pathway_fc, priority_var_int_cr)
cr_sn <- get_mixed_effects_stats(scale(sn) ~ scale(reverse_cr) + (1|id) + study, data = long_cr_pathway_fc, priority_var_int_cr)


# Cross-sectional analyses
cr_dmn_bl <- get_regression_stats(scale(dmn_baseline) ~ scale(reverse_cr) + study, data = clean_data, priority_var_int_cr)
cr_dmn_fu <- get_regression_stats(scale(dmn_followup) ~ scale(reverse_cr) + study, data = clean_data, priority_var_int_cr)

cr_ec_bl <- get_regression_stats(scale(ec_baseline) ~ scale(reverse_cr) + study, data = clean_data, priority_var_int_cr)
cr_ec_fu <- get_regression_stats(scale(ec_followup) ~ scale(reverse_cr) + study, data = clean_data, priority_var_int_cr)

cr_sn_bl <- get_regression_stats(scale(sn_baseline) ~ scale(reverse_cr) + study, data = clean_data, priority_var_int_cr)
cr_sn_fu <- get_regression_stats(scale(sn_followup) ~ scale(reverse_cr) + study, data = clean_data, priority_var_int_cr)
```

With functional connectivity, there was no evidence that ECN or DMN connectivity moderated the relationship between adjusted hippocampal volume and memory (ECN: _p_ = `r format_p_value(cr_ecn_stats$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_ecn_stats", "adjusted_p_value"])`, DMN: _p_ = `r format_p_value(cr_dmn_stats$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_dmn_stats", "adjusted_p_value"])`. Additionally, there was no evidence of a relationship between our CR variable and ECN or DMN connectivity (ECN: _p_ = `r format_p_value(cr_ec$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_ec", "adjusted_p_value"])`, DMN: _p_ = `r format_p_value(cr_dmn$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_dmn", "adjusted_p_value"])`. There was evidence, however, than anterior SN connectivity moderated the relationship between adjusted hippocampal volume and memory such that higher anterior SN connectivity values reduced the positive relationship between memory and adjusted hippocampal volume (β = `r round(cr_sn_stats$coef, 3)`, _p_ = `r format(format_p_value(cr_sn_stats$p_value), scientific = FALSE)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_sn_stats", "adjusted_p_value"])`, marginal r² = `r round(cr_sn_stats$marginal_r2, 3)`, conditional r² = `r round(cr_sn_stats$conditional_r2, 3)`). There was also evidence of a positive relationship between our CR variable and anterior SN connectivity (β = `r round(cr_sn$coef, 3)`, _p_ = `r format(format_p_value(cr_sn$p_value), scientific = FALSE)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_sn", "adjusted_p_value"])`, marginal r² = `r round(cr_sn$marginal_r2, 3)`, conditional r² = `r round(cr_sn$conditional_r2, 3)`)..

Figure 5 panel A depicts how the relationship between memory and adjusted hippocampal volume varies by anterior SN connectivity with participants divided into a low anterior SN connectivity and high anterior SN connectivity groups.  Panel B depicts the relationship between our CR variable and anterior SN connectivity in panel B. All longitudinal functional connectivity results are summarized in Table 6. Cross-sectionally, there were no significant interactions between functional connectivity and adjusted hippocampal volume regressed on memory or any associations between our CR variable and functional connectivity (Supplementary Table 2).

```{r echo=FALSE, message=FALSE, warning=FALSE}
# Center the variables
long_cr_pathway_fc$reverse_cr_centered = scale(long_cr_pathway_fc$reverse_cr)
long_cr_pathway_fc$sn_centered = scale(long_cr_pathway_fc$sn)

# Fit the mixed-effects model
model_lme_cr_sn <- lmer(sn_centered ~ reverse_cr_centered + (1 | id) + study + time, long_cr_pathway_fc)

reverse_cr_seq <- seq(
  from = min(long_cr_pathway_fc$reverse_cr_centered, na.rm = TRUE),
  to   = max(long_cr_pathway_fc$reverse_cr_centered, na.rm = TRUE),
  length.out = 100
)

# Generate predictions for each CR groupHarmless7-Tartar-Dust
# Divides the low and high CR groups into 100 reverse_cr bands and,
# using the LME model from above, predicts their memory for each of the reverse_cr bands
em_2group <- emmeans(
  model_lme_cr_sn,
  specs = ~ reverse_cr_centered,
  at    = list(reverse_cr_centered = reverse_cr_seq),
  reff  = 0
)
...
predictions <- summary(em_2group) %>%
  ...
  mutate(reverse_cr_centered = as.numeric(reverse_cr_centered))

# Create the plot
p1 <- ggplot() +
  geom_point(data = long_cr_pathway_fc, aes(x = reverse_cr_centered, y = sn_centered), color = "grey", size = 1, alpha = 0.8) + 
  geom_ribbon(data = predictions, aes(x = reverse_cr_centered, ymin = lower_bound, ymax = upper_bound), alpha = 0.3) + 
  geom_line(data = predictions, aes(x = reverse_cr_centered, y = predicted), color = "#129990", size = 1.2) +  
  labs(
    x = "Cognitive Reserve Variable", 
    y = "Salience Network Connectivty", 
    color = "Group", 
    fill = "Group", 
...
# Center the variables
long_clean_data_hc_mem_fc$memory_centered = scale(long_clean_data_hc_mem_fc$memory)
long_clean_data_hc_mem_fc$sn_centered = scale(long_clean_data_hc_mem_fc$sn)
long_clean_data_hc_mem_fc$adj_HIP_total_centered = scale(long_clean_data_hc_mem_fc$adj_HIP_total)

# Fit the mixed-effects model
long_clean_data_hc_mem_fc$sn_group <- cut(long_clean_data_hc_mem_fc$sn_centered,
  breaks = quantile(long_clean_data_hc_mem_fc$sn_centered, probs = seq(0, 1, (1 / 2)), na.rm = TRUE),
  include.lowest = TRUE,
  labels = c("Low SN Connectivity", "High SN Connectivity")
)

model_res_lme_cr <- lmer((memory_centered) ~ (sn_group) * (adj_HIP_total_centered) + (1|id) + study + time + sex + edu, long_clean_data_hc_mem_fc)

adj_HIP_total_centered_seq <- seq(min(long_clean_data_hc_mem_fc$adj_HIP_total_centered), max(long_clean_data_hc_mem_fc$adj_HIP_total_centered), length.out = 100)
...
em_2group <- emmeans(
  model_res_lme_cr,
  specs = ~ adj_HIP_total_centered * sn_group,
  at    = list(adj_HIP_total_centered = adj_HIP_total_centered_seq),
  reff  = 0
)
...
predictions <- summary(em_2group) %>%
  ...
  mutate(adj_HIP_total_centered = as.numeric(adj_HIP_total_centered))

# Define colors for each CR group
palette <- c("High SN Connectivity" = "#A4CCD9", "Low SN Connectivity" = "#FFAAAA")

# Create the plot
p2 <- ggplot() +
  geom_line(data = long_clean_data_hc_mem_fc, aes(x = adj_HIP_total_centered, y = memory_centered, group = id), color = "lightgray", alpha = 0.5) + # Plot individual trajectories
  geom_point(data = long_clean_data_hc_mem_fc, aes(x = adj_HIP_total_centered, y = memory_centered), color = "gray", size = 1, alpha = 0.8) + # Add scatter points
  geom_ribbon(data = predictions, aes(x = adj_HIP_total_centered, ymin = lower_bound, ymax = upper_bound, fill = as.factor(sn_group)), alpha = 0.3) + # Add CIs
  geom_line(data = predictions, aes(x = adj_HIP_total_centered, y = predicted, color = as.factor(sn_group)), size = 1.2) + # Plot predicted lines for each group
...
long_fc <- data.frame(
  ...
  "**DMN Connectivity * Adjusted Hippocampal Volume**" = c(
    ...
    "**Cognitive Reserve**", "_Coefficient (95% CI)_",
  ...
  " " = c(
    "_Marginal R²_", round(cr_dmn_stats$marginal_r2, 3),
    "", "_Marginal R²_", round(cr_dmn$marginal_r2, 3)
  ),
  ...
  " " = c(
    "_Marginal R²_", round(cr_ecn_stats$marginal_r2, 3),
    "", "_Marginal R²_", round(cr_ec$marginal_r2, 3)
  ),
  ...
  " " = c(
    "_Marginal R²_", round(cr_sn_stats$marginal_r2, 3),
    "", "_Marginal R²_", round(cr_sn$marginal_r2, 3)
  )
)
```

__Table 6__: _Regressions with Cognitive Reserve and Functional Connectivity_
```{r tab7, echo=FALSE} 
# Format and print the table
kable(long_fc, format = "markdown", col.names = c(" ", "**DMN Connectivity * Adjusted Hippocampal Volume***", " ", " ", "**ECN Connectivity * Adjusted Hippocampal Volume**", " ", " ", "**Anterior SN Connectivity * Adjusted Hippocampal Volume****", " ", " "))
```
...
fc_long_cr_ecn <- list(cr_ecn_stats, cr_ec)

# DMN and CR Analyses Longitudinal
fc_long_cr_dmn <- list(cr_dmn_stats, cr_dmn)

# SN and CR Analyses Longitudinal
fc_long_cr_sn <- list(cr_sn_stats, cr_sn)

# ECN and CR Analyses Cross-sectional
fc_cs_cr_ecn <- list(cr_ec_bl, cr_ec_fu, cr_ecn_stats_bl, cr_ecn_stats_fu)

# DMN and CR Analyses Cross-sectional
fc_cs_cr_dmn <- list(cr_dmn_bl, cr_dmn_fu, cr_dmn_stats_bl, cr_dmn_stats_fu)

# SN and CR Analyses Cross-sectional
fc_cs_cr_sn <- list(cr_sn_bl, cr_sn_fu, cr_sn_stats_bl, cr_sn_stats_fu)
...
fc_long_cr_ecn_p_values <- adjust_p_values(
  fc_long_cr_ecn,
  c("cr_ecn_stats", "cr_ec")
)
fc_long_cr_dmn_p_values <- adjust_p_values(
  fc_long_cr_dmn,
  c("cr_dmn_stats", "cr_dmn")
)
fc_long_cr_sn_p_values <- adjust_p_values(
  fc_long_cr_sn,
  c("cr_sn_stats", "cr_sn")
)
fc_cs_cr_ecn_p_values <- adjust_p_values(
  fc_cs_cr_ecn,
  c("cr_ecn_stats_bl", "cr_ecn_stats_fu", "cr_ec_bl", "cr_ec_fu")
)
fc_cs_cr_dmn_p_values <- adjust_p_values(
  fc_cs_cr_dmn,
  c("cr_dmn_stats_bl", "cr_dmn_stats_fu", "cr_dmn_bl", "cr_dmn_fu")
)
fc_cs_cr_sn_p_values <- adjust_p_values(
  fc_cs_cr_sn,
  c("cr_sn_stats_bl", "cr_sn_stats_fu", "cr_sn_bl", "cr_sn_fu")
)
...
p_value_df <- rbind(
  ...
  fc_cs_cr_ecn_p_values, 
  fc_cs_cr_dmn_p_values, 
  fc_cs_cr_sn_p_values,
