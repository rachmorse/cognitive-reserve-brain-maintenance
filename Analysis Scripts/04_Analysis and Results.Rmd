...
int_vars <- c(
  "scale(reverse_cr):scale(edu):scale(age)"
)

edu_cr <- get_regression_stats(scale(edu) ~ scale(reverse_cr) + study, data = clean_data, vars)

edu_cr_int <- get_mixed_effects_stats(scale(memory) ~ scale(reverse_cr) * scale(edu) * scale(age) + (1|id) + study, data = long_clean_data, int_vars)
```

__**Cognitive Reserve Associations with Education and Functional Connectivity:**__

There were no significant relationships between CR (_p_ = `r format_p_value(edu_cr$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_cr", "adjusted_p_value"])`) and education, nor was there a significant interaction between CR, education and age on memory (_p_ = `r format_p_value(edu_cr_int$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_cr_int", "adjusted_p_value"])`; Table 5). 
...
edu_table_data <- data.frame(
  ...
  "**Cognitive Reserve * Education * Age**" = c(
    "_Coefficient (95% CI)_",
...
```{r tab6, echo=FALSE} 
# Format and print the table
kable(edu_table_data, format = "markdown", col.names = c(" ", "**Cognitive Reserve**", " ", " ", " ", "**Cognitive Reserve * Education * Age**", " ", " "))
```
...
priority_var_int_cr <- c(
  "scale(reverse_cr):scale(dmn):scale(age)",
  "scale(reverse_cr):scale(ec):scale(age)",
  "scale(reverse_cr):scale(sn):scale(age)",
...
cr_dmn <- get_mixed_effects_stats(scale(memory) ~ scale(reverse_cr) * scale(dmn) * scale(age) + (1|id) + study, data = long_clean_data, priority_var_int_cr)
cr_ec <- get_mixed_effects_stats(scale(memory) ~ scale(reverse_cr) * scale(ec) * scale(age) + (1|id) + study, data = long_clean_data, priority_var_int_cr)
cr_sn <- get_mixed_effects_stats(scale(memory) ~ scale(reverse_cr) * scale(sn) * scale(age) + (1|id) + study, data = long_clean_data, priority_var_int_cr)
...
With functional connectivity, there was no evidence of a significant association between CR and anterior SN annual change (_p_ = `r format_p_value(cr_sn_stats$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_sn_stats", "adjusted_p_value"])`). A marginal effect of CR on DMN (_p_ = `r format_p_value(cr_dmn_stats$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_dmn_stats", "adjusted_p_value"])`) and ECN annual change (_p_ = `r format_p_value(cr_ecn_stats$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_ecn_stats", "adjusted_p_value"])`) did not survive correction for multiple comparisons. There was no evidence of an interaction between CR, age and anterior SN connectivity (_p_ = `r format_p_value(cr_sn$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_sn", "adjusted_p_value"])`) or CR, age and DMN connectivity (_p_ = `r format_p_value(cr_dmn$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_dmn", "adjusted_p_value"])`) on memory. However, there was a significant interaction between CR and ECN connectivity on the relationship between age and memory (β = `r round(cr_ec$coef, 3)`, _p_ = `r format(format_p_value(cr_ec$p_value), scientific = FALSE)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_ec", "adjusted_p_value"])`, marginal r² = `r round(cr_ec$marginal_r2, 3)`, conditional r² = `r round(cr_ec$conditional_r2, 3)`). Specifically, individuals with higher CR and increased ECN connectivity showed less memory decline as they aged. 

Figure 5 depicts how the relationship between memory and age vary by CR and ECN Connectivity with panel A depicting low CR individuals and panel B depicting high CR individuals. All longitudinal functional connectivity results are summarized in Table 6. Cross-sectionally, there were no significant associations between CR and functional connectivity (Supplementary Table 2).
...
long_clean_data_fc <- long_clean_data %>%
  filter(!is.na(sn)) # Filter to people with two timepoints of FC data

long_clean_data_fc_mem <- long_clean_data %>%
  filter(!is.na(sn) & !is.na(memory) & !is.na(sex) & !is.na(edu)) # Filter to people with two timepoints of FC and mem

long_clean_data_hc_mem <- long_clean_data %>%
  filter(!is.na(adj_HIP_total) & !is.na(memory) & !is.na(sex) & !is.na(edu))

long_clean_data_hc_mem_fc <- long_clean_data %>%
  filter(!is.na(adj_HIP_total) & !is.na(memory) & !is.na(sex) & !is.na(edu) & !is.na(sn))

long_clean_data_mem <- long_clean_data %>%
  filter(!is.na(memory) & !is.na(sex) & !is.na(edu))

long_cr_pathway_fc_mem <- long_clean_data_fc_mem %>%
  filter(pathway == "CR Pathway") 

long_cr_pathway_fc <- long_clean_data_fc_mem %>%
  filter(pathway == "CR Pathway") 

# check if this is true in the CR group and then verify that it is NOT true in the BM group
# so my suggestion is that we actually keep it simple and just run these models here without bothering to do interactions to address Anders' criticism 

# Maybe would need to run this but the long is prob fine since its used in the creation of the variables
summary(lmer(memory ~ adj_HIP_total + (1|id) + study + age + time + sex + edu, long_clean_data_hc_mem))

# Then the CR / HC interaction - just a note that in order to run this though we should run the other LME right? 
summary(lmer(memory ~ adj_HIP_total * reverse_cr + (1|id) + study + age + time + sex + edu, long_clean_data_hc_mem))

# Then the edu model so maybe this would be to remove because it a bit of a tangent for this paper anyway
summary(lmer(scale(memory) ~ scale(edu) + (1|id) + study + time + sex, long_clean_data_mem))
summary(lmer(scale(memory) ~ scale(edu) * scale(age) + (1|id) + study + time + sex, long_clean_data_mem))
summary(lmer(scale(memory) ~ scale(edu) * scale(adj_HIP_total) + (1|id) + age + study + time + sex, long_clean_data_mem))

# Then CR / edu specifically with the aging CR-like group (keeping it an lm because people only have one edu and one cR value)
summary(lmer(edu ~ reverse_cr + (1|id) + age + study + time + sex, long_cr_pathway))

# Maybe check memory first to find which ones are important
# Then check if sn is related to memory? Only in the whole cohort not the CR one so I don't know maybe you can use the whole cohort here 
summary(lmer(scale(memory) ~ scale(sn) + (1|id) + study + age + time + sex + edu, long_clean_data_fc_mem))
summary(lmer(scale(memory) ~ scale(ec) + (1|id) + study + age + time + sex + edu, long_clean_data_fc_mem))
summary(lmer(scale(memory) ~ scale(dmn) + (1|id) + study + age + time + sex + edu, long_clean_data_fc_mem))

# Then because sn is related can do 
summary(lmer(scale(memory) ~ scale(sn) * scale(age) + (1|id) + study + time + sex + edu, long_clean_data_fc_mem))
summary(lmer(scale(memory) ~ scale(ec) * scale(age) + (1|id) + study + time + sex + edu, long_clean_data_fc_mem))
summary(lmer(scale(memory) ~ scale(dmn) * scale(age) + (1|id) + study + time + sex + edu, long_clean_data_fc_mem))

# FC as CR
summary(lmer(scale(memory) ~ scale(sn) * scale(adj_HIP_total) + (1|id) 
             + study + age + sex, long_clean_data_hc_mem_fc))
summary(lmer(scale(memory) ~ scale(ec) * scale(adj_HIP_total) + (1|id) 
             + study + age + sex, long_clean_data_hc_mem_fc))
summary(lmer(scale(memory) ~ scale(dmn) * scale(adj_HIP_total) + (1|id) 
             + study + age + sex + edu, long_clean_data_hc_mem_fc))

# Then CR / FC
summary(lmer(ec ~ reverse_cr + (1|id) + study + time, long_cr_pathway_fc))
summary(lmer(sn ~ reverse_cr + (1|id) + study + time, long_cr_pathway_fc))
summary(lmer(dmn ~ reverse_cr + (1|id) + study + time, long_cr_pathway_fc))

# Then maybe make sure this is not the case for the BM group - unclear whether this should be CR or BM - BM right?
long_bm_pathway <- long_clean_data_fc %>%
  filter(pathway == "BM Pathway") 

summary(lmer(ec ~ reverse_cr + (1|id) + study + time, long_bm_pathway))
summary(lmer(sn ~ reverse_cr + (1|id) + study + time, long_bm_pathway))
summary(lmer(dmn ~ reverse_cr + (1|id) + study + time, long_bm_pathway))

summary(lmer(ec ~ reverse_bm + (1|id) + study + time, long_bm_pathway))
summary(lmer(sn ~ reverse_bm + (1|id) + study + time, long_bm_pathway))
summary(lmer(dmn ~ reverse_bm + (1|id) + study + time, long_bm_pathway))

# Example: pvals is your vector of p-values
pvals <- c(0.028267, 0.231141, 0.11711, 0.0112, 0.0247)
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# Fit the mixed-effects model
model_lme_cr_sn <- lmer(sn ~ reverse_cr + (1 | id) + study + time, long_cr_pathway)
...
# Create the plot
p1 <- ggplot() +
  geom_point(data = long_cr_pathway, aes(x = reverse_cr, y = sn), color = "grey", size = 1, alpha = 0.8) + 
  geom_ribbon(data = predictions, aes(x = reverse_cr, ymin = lower_bound, ymax = upper_bound), alpha = 0.3) + 
  geom_line(data = predictions, aes(x = reverse_cr, y = predicted), color = "#129990", size = 1.2) +  
  labs(
    x = "Cognitive Reserve", 
    y = "Salience Network", 
    color = "Group", 
...
# Fit the mixed-effects model
long_clean_data_fc_mem$sn_group <- cut(long_clean_data_fc_mem$sn,
  breaks = quantile(long_clean_data_fc_mem$sn, probs = seq(0, 1, (1 / 2)), na.rm = TRUE),
  include.lowest = TRUE,
  labels = c("Low SN", "High SN")
)

model_res_lme_cr <- lmer((memory) ~ (sn_group) * (age) + (1|id) + study + time + sex + edu, long_clean_data_fc_mem)
...
palette <- c("High SN" = "#A4CCD9", "Low SN" = "#FFAAAA")

# Create the plot
p2 <- ggplot() +
  geom_line(data = long_clean_data_fc_mem, aes(x = age, y = memory, group = id), color = "lightgray", alpha = 0.5) + # Plot individual trajectories
  geom_point(data = long_clean_data_fc_mem, aes(x = age, y = memory), color = "gray", size = 1, alpha = 0.8) + # Add scatter points
  geom_ribbon(data = predictions, aes(x = age, ymin = lower_bound, ymax = upper_bound, fill = as.factor(sn_group)), alpha = 0.3) + # Add CIs
  geom_line(data = predictions, aes(x = age, y = predicted, color = as.factor(sn_group)), size = 1.2) + # Plot predicted lines for each group
...
# Then the edu model so maybe this would be to remove because it a bit of a tangent for this paper anyway
summary(lmer(memory ~ edu + (1|id) + age + study + time + sex, long_clean_data_mem))

# Then the CR / HC interaction
summary(lmer(memory ~ adj_HIP_total * reverse_cr + (1|id) + study + age + time + sex + edu, long_clean_data_hc_mem))

# Then CR / edu specifically with the aging CR-like group (keeping it an lm because people only have one edu and one cR value)
summary(lm(edu ~ reverse_cr + age + study + time + sex, long_cr_pathway))
...
long_fc <- data.frame(
  ...
  "**DMN Baseline**" = c(
    ...
    "**DMN Connectivity * Cognitive Reserve * Age**", "_Coefficient (95% CI)_",
  ...
  "**ECN Annual Change**" = c(
    ...
    "**ECN Connectivity * Cognitive Reserve * Age **", "_Coefficient (95% CI)_",
  ...
  "**Anterior SN Annual Change**" = c(
    ...
    "**Anterior SN Connectivitym * Cognitive Reserve * Age**", "_Coefficient (95% CI)_",
...
fc_long_cr <- list(cr_dmn_stats, cr_ecn_stats, cr_sn_stats)

# Functional Connectivity, CR/BM, age, memory Analyses Longitudinal
fc_long_cr_age_mem <- list(cr_dmn, cr_ec, cr_sn)
...
# Functional Connectivity, CR/BM, age, memory Analyses Cross-sectional
fc_cs_cr_age_mem <- list(
  cr_dmn_stats_bl, cr_dmn_stats_fu,
  cr_ecn_stats_bl, cr_ecn_stats_fu,
  cr_sn_stats_bl,  cr_sn_stats_fu
)
...
fc_long_cr_p_values <- adjust_p_values(
  fc_long_cr,
  c("cr_dmn_stats", "cr_ecn_stats", "cr_sn_stats")
)
fc_long_cr_age_mem_p_values <- adjust_p_values(
  fc_long_cr_age_mem,
  c("cr_dmn", "cr_ec", "cr_sn")
)
...
fc_cs_cr_age_mem_p_values <- adjust_p_values(
  fc_cs_cr_age_mem,
  c(
    "cr_dmn_bl", "cr_dmn_fu",
    "cr_ec_bl", "cr_ec_fu", 
...
p_value_df <- rbind(
  ...
  cr_bm_age_p_values,
  fc_long_cr_p_values,
  fc_long_cr_age_mem_p_values,
  fc_cs_cr_p_values,
  fc_cs_cr_age_mem_p_values,
