...
```
<small>(A) The association between memory and age with individual trajectories and a linear model displaying a coefficient of `r round(mem_age_unscaled$coef, 3)` (scaled: `r round(mem_age_lin$coef, 3)`) and confidence intervals. (B) The association between adjusted hippocampal volume and age with individual trajectories with a generalized additive model (EDF = `r round(summary_hc$s.table["s(age)", "edf"], 3)`) and confidence intervals.</small>
