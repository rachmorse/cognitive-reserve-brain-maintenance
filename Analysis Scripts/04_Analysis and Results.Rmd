...
Illustration of participants classified as aging either BM-like (left panel) or CR-like (CR) (right panel). The vertical axis indicates the BM or CR value, which is derived from each individual’s proximity to an ideal BM or CR pattern of hippocampal and memory change (see Methods and Figure 1 for details). The color gradient reflects the BM or CR value within each subgroup. 
...
```{r tab6, echo=FALSE} 
# Format and print the table
kable(edu_table_data, format = "markdown", col.names = c(" ", "**Brain Maintenance**", " ", " ", "**Cognitive Reserve**", " ", " "))
```
...
There was no evidence of a significant association between BM and DMN (_p_ = `r format_p_value(bm_dmn_stats$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "bm_dmn_stats", "adjusted_p_value"])`) or ECN annual change (_p_ = `r format_p_value(bm_ecn_stats$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "bm_ecn_stats", "adjusted_p_value"])`). A marginal effect of BM on anterior SN annual change did not survive multiple-comparison correction (_p_ = `r format_p_value(bm_sn_stats$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "bm_sn_stats", "adjusted_p_value"])`). Interactions between BM and both ECN (_p_ = `r format_p_value(bm_ec$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "bm_ec", "adjusted_p_value"])`) and anterior SN annual change (_p_ = `r format_p_value(bm_sn$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "bm_sn", "adjusted_p_value"])`) regressed on memory annual change were not significant. After multiple-comparison correction, the interaction between BM and DMN annual change regressed on memory annual change was not significant (_p_ = `r format_p_value(bm_dmn$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "bm_dmn", "adjusted_p_value"])`). Cross-sectionally, there were no significant associations between BM and functional connectivity (Supplementary Table 2).
...
## 1) Fit two separate SN models (Low CR and High CR)
fit_sn_low <- lm(memory_slopes ~ sn_slopes + study,
  data = subset(clean_data, reverse_cr_group == "Low CR")
)
fit_sn_mid <- lm(memory_slopes ~ sn_slopes + study,
  data = subset(clean_data, reverse_cr_group == "Mid CR")
)
fit_sn_high <- lm(memory_slopes ~ sn_slopes + study,
  data = subset(clean_data, reverse_cr_group == "High CR")
)

## 2) Create reference grids for each model
rg_sn_low <- ref_grid(fit_sn_low,
  at = list(sn_slopes = seq(-0.1, 0.15, by = 0.01))
)
rg_sn_mid <- ref_grid(fit_sn_mid,
  at = list(sn_slopes = seq(-0.1, 0.15, by = 0.01))
)
rg_sn_high <- ref_grid(fit_sn_high,
  at = list(sn_slopes = seq(-0.1, 0.15, by = 0.01))
)

## 3) Get the predicted means for each grid
em_sn_low <- emmeans(rg_sn_low, "sn_slopes") # partial effect for fit_sn_low
em_sn_mid <- emmeans(rg_sn_mid, "sn_slopes")
em_sn_high <- emmeans(rg_sn_high, "sn_slopes")

## Convert to data frames for ggplot
df_sn_low <- as.data.frame(em_sn_low)
df_sn_mid <- as.data.frame(em_sn_mid)
df_sn_high <- as.data.frame(em_sn_high)

df_sn_low$reverse_cr_group <- "Low CR"
df_sn_mid$reverse_cr_group <- "Mid CR"
df_sn_high$reverse_cr_group <- "High CR"

## 4) Plot each line and CI ribbon
sn_int <- ggplot() +
  geom_point(
    data = clean_data,
    aes(x = sn_slopes, y = memory_slopes, color = reverse_cr_group),
    alpha = 0.5
  ) +
  scale_color_manual(
    values = c("High CR" = "#493D9E", "Low CR" = "#F0A04B", "Mid CR" = "lightgrey")
  ) +
  geom_line(
    data = df_sn_low,
    aes(x = sn_slopes, y = emmean, color = reverse_cr_group),
    size = 1.1
  ) +
  geom_line(
    data = df_sn_mid,
  ...
  geom_ribbon(
    data = df_sn_high,
    aes(x = sn_slopes, ymin = lower.CL, ymax = upper.CL),
    fill = "#B2A5FF", # This “hard‐codes” a color instead of mapping
    alpha = 0.3
  ) +
  geom_line(
    data = df_sn_high,
...
## 1) Fit two separate EC models (Low CR and High CR)
fit_ec_low <- lm(memory_slopes ~ ec_slopes + study,
  data = subset(clean_data, reverse_cr_group == "Low CR")
)
fit_ec_mid <- lm(memory_slopes ~ ec_slopes + study,
  data = subset(clean_data, reverse_cr_group == "Mid CR")
)
fit_ec_high <- lm(memory_slopes ~ ec_slopes + study,
  data = subset(clean_data, reverse_cr_group == "High CR")
)

## 2) Create reference grids for each model
rg_ec_low <- ref_grid(fit_ec_low,
  at = list(ec_slopes = seq(-0.1, 0.15, by = 0.01))
)
rg_ec_mid <- ref_grid(fit_ec_mid,
  at = list(ec_slopes = seq(-0.1, 0.15, by = 0.01))
)
rg_ec_high <- ref_grid(fit_ec_high,
  at = list(ec_slopes = seq(-0.1, 0.15, by = 0.01))
)

## 3) Get the predicted means for each grid
em_ec_low <- emmeans(rg_ec_low, "ec_slopes") # partial effect for fit_ec_low
em_ec_mid <- emmeans(rg_ec_mid, "ec_slopes")
em_ec_high <- emmeans(rg_ec_high, "ec_slopes")

## Convert to data frames for ggplot
df_ec_low <- as.data.frame(em_ec_low)
df_ec_mid <- as.data.frame(em_ec_mid)
df_ec_high <- as.data.frame(em_ec_high)

df_ec_low$reverse_cr_group <- "Low CR"
df_ec_mid$reverse_cr_group <- "Mid CR"
df_ec_high$reverse_cr_group <- "High CR"

## 4) Plot each line and CI ribbon
ec_int <- ggplot() +
  geom_point(
    data = clean_data,
    aes(x = ec_slopes, y = memory_slopes, color = reverse_cr_group),
    alpha = 0.5
  ) +
  scale_color_manual(
    values = c("High CR" = "#493D9E", "Low CR" = "#F0A04B", "Mid CR" = "lightgrey")
  ...
  geom_line(
    data = df_ec_low,
    aes(x = ec_slopes, y = emmean, color = reverse_cr_group),
    size = 1.1
  ) +
  geom_line(
    data = df_ec_mid,
  ...
  geom_ribbon(
    data = df_ec_low,
    aes(x = ec_slopes, ymin = lower.CL, ymax = upper.CL),
    fill = "#FADA7A",
    alpha = 0.3
  ) +
  geom_ribbon(
...
mid_cr <- clean_data %>%
  filter(reverse_cr_group == "Mid CR")
...
mid_cr_sn <- get_regression_stats(scale(memory_slopes) ~ scale(sn_slopes) + study, data = mid_cr, vars)
mid_cr_sn_unscaled <- get_regression_stats((memory_slopes) ~ (sn_slopes) + study, data = mid_cr, vars_unscale)
...
mid_cr_ec <- get_regression_stats(scale(memory_slopes) ~ scale(ec_slopes) + study, data = mid_cr, vars)
mid_cr_ec_unscaled <- get_regression_stats((memory_slopes) ~ (ec_slopes) + study, data = mid_cr, vars_unscale)
...
<small>(A) The association between anterior SN annual change and memory annual change, adjusted for cohort/MRI scanner, depicted divided into three groups: high CR (blue), mid CR (grey) and low CR (orange). There are linear models for each group with the high CR group displaying a coefficient of `r round(high_cr_sn_unscaled$coef, 3)` (scaled: `r round(high_cr_sn$coef, 3)`), the mid CR group displaying a coefficient of `r round(mid_cr_sn_unscaled$coef, 3)` (scaled: `r round(mid_cr_sn$coef, 3)`), and the low CR group displaying a coefficient of `r round(low_cr_sn_unscaled$coef, 3)` (scaled: `r round(low_cr_sn$coef, 3)`) and confidence intervals. (B) The association between ECN annual change and memory annual change, adjusted for cohort/MRI scanner, depicted divided into the three groups with the high CR group displaying a coefficient of `r round(high_cr_ec_unscaled$coef, 3)` (scaled: `r round(high_cr_ec$coef, 3)`), the mid CR group displaying a coefficient of `r round(mid_cr_ec_unscaled$coef, 3)` (scaled: `r round(mid_cr_ec$coef, 3)`), and the low CR group displaying a coefficient of `r round(low_cr_ec_unscaled$coef, 3)` (scaled: `r round(low_cr_ec$coef, 3)`). Abbreviations: ECN Executive Control Network, SN Salience Network, CR Cognitive Reserve, BM Brain Maintenance</small>
...
education_bm_p_values <- adjust_p_values(
  education_bm_models,
  c("edu_bm", "edu_bm_int")
)
...
fc_long_bm_p_values <- adjust_p_values(
  fc_long_bm,
  c("bm_dmn_stats", "bm_dmn", "bm_ecn_stats", "bm_ec", "bm_sn_stats", "bm_sn")
)
...
fc_cs_bm_p_values <- adjust_p_values(
  fc_cs_bm,
  c(
    "bm_dmn_bl", "bm_dmn_fu", "bm_dmn_stats_bl", "bm_dmn_stats_fu",
    "bm_ec_bl", "bm_ec_fu", "bm_ecn_stats_bl", "bm_ecn_stats_fu",
