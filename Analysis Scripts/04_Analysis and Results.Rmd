...
clean_data <- clean_data %>%
  mutate(
    ################################### REMOVE DEPENDING ON INDEX FIGURE
...
#   geom_jitter(aes(color = pathway), shape = 21, size = 2.5, width = 0, height = 1) +
...
A figure depicting participants as either on the BM or CR pathway. The x-axis represents the total CR value for participants on the CR pathway and the total BM value for participants on the BM pathway. The y-axis represents the density of participants.
...
For participants on the CR path, we then found a significant interaction between CR and age (β = `r round(cr_age_lme$coef, 3)`, _p_ = `r format_p_value(cr_age_lme$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_age_lme", "adjusted_p_value"])`, r² = `r round(cr_age_lme$r2, 3)`) and, for participants on the BM path, a significant interaction between BM and age (β = `r round(bm_age_lme$coef, 3)`, _p_ = `r format_p_value(bm_age_lme$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "bm_age_lme", "adjusted_p_value"])`, r² = `r round(bm_age_lme$r2, 3)`) on memory (Table 5). 
