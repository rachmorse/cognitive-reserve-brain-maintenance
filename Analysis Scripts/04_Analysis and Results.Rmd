...
# Model with age interactions - alternative to interaction from lm
model <- lmer(scale(memory) ~ scale(reverse_cr) * scale(adj_HIP_total) + (1|id) + study + scale(age_baseline) + scale(mem_time) + sex + scale(edu), data=long_clean_data)
summary(model)

summary(lm(scale(dmn_slopes) ~ scale(reverse_cr) + study, data = clean_data))
summary(lm(scale(ec_slopes) ~ scale(reverse_cr) + study, data = clean_data))
summary(lm(scale(sn_slopes) ~ scale(reverse_cr) + study, data = clean_data))

summary(lmer(scale(memory) ~ scale(reverse_cr) * scale(ec) * scale(age) + (1|id) + study, data=long_clean_data))
summary(lmer(scale(memory) ~ scale(reverse_cr) * scale(sn) * scale(age) + (1|id) + study, data=long_clean_data))
summary(lmer(scale(memory) ~ scale(reverse_cr) * scale(dmn) * scale(age) + (1|id) + study, data=long_clean_data))

# p-vals split by type of model (eg one for direct and one for relationship with age)
pvals <- c(0.0410, 0.0372, 0.489089) # Direct
pvals <- c(0.01569, 0.44351, 0.06564) # Age
...
# By this logic would need to test whether edu has an interaction with CR and age on mem
summary(lmer(scale(memory) ~ scale(reverse_cr) * scale(edu) * scale(age) + (1|id) + study, data=long_clean_data))
...
clean_data$ec_group <- cut(clean_data$ec_slopes,
  breaks = quantile(clean_data$ec_slopes, probs = seq(0, 1, (1 / 2)), na.rm = TRUE),
  include.lowest = TRUE,
  labels = c("Low ECN", "High ECN")
)

clean_data$dmn_group <- cut(clean_data$dmn_slopes,
  breaks = quantile(clean_data$dmn_slopes, probs = seq(0, 1, (1 / 2)), na.rm = TRUE),
  include.lowest = TRUE,
  labels = c("Low DMN", "High DMN")
)
...
long_clean_data_figure <- clean_data %>%
  pivot_longer(
    cols = matches("(.+)\\.(\\d+)$"),             
...
long_clean_data_figure_low_cr <- long_clean_data_figure %>% 
  filter(reverse_cr_group == "Low CR") %>% 
  filter(!is.na(reverse_cr_group)) %>% 
  filter(!is.na(ec_group))

long_clean_data_figure_high_cr <- long_clean_data_figure %>% 
  filter(reverse_cr_group == "High CR") %>% 
  filter(!is.na(reverse_cr_group)) %>% 
  filter(!is.na(ec_group))


p1 <- ggplot(long_clean_data_figure_low_cr, aes(x = age, y = memory, color = ec_group)) +
  geom_line(data = long_clean_data_figure_low_cr, aes(x = age, y = memory, group = id), color = "lightgray", alpha = 0.5) +
  geom_point(data = long_clean_data_figure_low_cr, aes(x = age, y = memory), color = "gray", size = 1, alpha = 0.8) +     
...
p2 <- ggplot(long_clean_data_figure_high_cr, aes(x = age, y = memory, color = ec_group)) +
  geom_line(data = long_clean_data_figure_high_cr, aes(x = age, y = memory, group = id), color = "lightgray", alpha = 0.5) +
  geom_point(data = long_clean_data_figure_high_cr, aes(x = age, y = memory), color = "gray", size = 1, alpha = 0.8) + 
  geom_smooth(method = "lm") + 
  labs(title = "High CR") + 
  theme_bw()

combined_test <- p1 + p2 + plot_layout(ncol = 2)
combined_test
```
