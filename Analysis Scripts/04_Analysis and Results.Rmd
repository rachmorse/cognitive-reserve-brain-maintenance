...
We then found a significant interaction between resilience and age on memory (β = `r round(resilience_lme$coef, 3)`, _p_ = `r format_p_value(resilience_lme$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "resilience_lme", "adjusted_p_value"])`, r² = `r round(resilience_lme$r2, 3)`) (Figure 5). We also found a significant interaction between CR and age (β = `r round(cr_age_lme$coef, 3)`, _p_ = `r format_p_value(cr_age_lme$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_age_lme", "adjusted_p_value"])`, r² = `r round(cr_age_lme$r2, 3)`) and BM and age (β = `r round(bm_age_lme$coef, 3)`, _p_ = `r format_p_value(bm_age_lme$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "bm_age_lme", "adjusted_p_value"])`, r² = `r round(bm_age_lme$r2, 3)`) on memory (Table 5). 
...
table_resilience <- data.frame(
  ...
  " " = c(
    "_R²_", round(resilience_lme$r2, 3)
  ),
...
# Fit the mixed-effects model
model_res_lme <- lmer(memory ~ age * resilience + (1 | id), data = long_data_vis)
...
# Divides the low and high resilience groups into 100 age bands and,
...
suppressMessages({
  high_res_age <- get_mixed_effects_stats(scale(memory) ~ scale(age) + (1 | id), data = high_resilience, vars)
  high_res_age_unscaled <- get_mixed_effects_stats((memory) ~ age + (1 | id), data = high_resilience, vars_unscale)

  low_res_age <- get_mixed_effects_stats(scale(memory) ~ scale(age) + (1 | id), data = low_resilience, vars)
  low_res_age_unscaled <- get_mixed_effects_stats((memory) ~ (age) + (1 | id), data = low_resilience, vars_unscale)
})
```
<small>(A) The association between memory and age depicted divided into two groups: high resilience (blue) and low resilience (orange). There are linear mixed effects models for each group with the high resilience group displaying a coefficient of `r round(high_res_age_unscaled$coef, 3)` (scaled: `r round(high_res_age$coef, 3)`) and the low resilience group displaying a coefficient of `r round(low_res_age_unscaled$coef, 3)` (scaled: `r round(low_res_age$coef, 3)`) and confidence intervals.</small>
