...
We then created the CR and BM index with participants classified as on the CR pathway (N = `r cr_pathway`/`r total_participants`) or the BM pathway (N = `r bm_pathway`/`r total_participants`) (Figure 4). The mean level of the calculated resilience variable was `r round(mean_total_cr_bm, 3)` (SD = `r round(sd_total_cr_bm, 3)`) and the mean absolute index value was `r round(mean_distance_midline, 3)` (SD = `r round(sd_distance_midline, 3)`).
...
resilience_lme <- get_mixed_effects_stats(
  scale(memory) ~ scale(age) * scale(total_cr_bm) + study + scale(age_baseline) + 
...
We then found a significant interaction between resilience and age regressed on memory (β = `r round(resilience_lme$coef, 3)`, _p_ = `r format_p_value(resilience_lme$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "resilience_lme", "adjusted_p_value"])`, r² = `r round(resilience_lme$r2, 3)`) (Figure 5). We also found a significant interaction between CR and age (β = `r round(cr_age_lme$coef, 3)`, _p_ = `r format_p_value(cr_age_lme$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_age_lme", "adjusted_p_value"])`, r² = `r round(cr_age_lme$r2, 3)`) and BM and age (β = `r round(bm_age_lme$coef, 3)`, _p_ = `r format_p_value(bm_age_lme$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "bm_age_lme", "adjusted_p_value"])`, r² = `r round(bm_age_lme$r2, 3)`) regressed on memory (Table 5). 

```{r echo=FALSE, message=FALSE, warning=FALSE}

# The interaction between age and resilience (t = `r round(resilience_lme$t_stat, 3)`,
# _p_ = `r format(resilience_lme$p_value, digits = 3, scientific = TRUE)`, _p_<sub>FDR</sub> = `r format(p_value_df[p_value_df$model_name == "resilience_lme", "adjusted_p_value"], digits = 3, scientific = TRUE)`) was stronger than that of age and BM (t = `r round(bm_age_lme$t_stat, 3)`, _p_ = `r format(bm_age_lme$p_value, digits = 3, scientific = TRUE)`, _p_<sub>FDR</sub> = `r format(p_value_df[p_value_df$model_name == "bm_age_lme", "adjusted_p_value"], digits = 3, scientific = TRUE)`) and that of age and CR (t = `r round(cr_age_lme$t_stat, 3)`, _p_ = `r format(cr_age_lme$p_value, digits = 3, scientific = TRUE)`, _p_<sub>FDR</sub> = `r format(p_value_df[p_value_df$model_name == "cr_age_lme", "adjusted_p_value"], digits = 3, scientific = TRUE)`).  
# Create the table
table_resilience <- data.frame(
  ...
  "**Resilience * Age**" = c(
    "_Coefficient (95% CI)_",
    paste0(round(resilience_lme$coef, 3), " (", round(resilience_lme$ci[1], 3), " to ", round(resilience_lme$ci[2], 3), ")")
  ),
  " " = c(
    "_Adjusted p-value_", format_p_value(p_value_df[p_value_df$model_name == "resilience_lme", "adjusted_p_value"])
  ),
  " " = c(
    "_Adjusted R²_", round(resilience_lme$r2, 3)
  ),
  ...
  " " = c(
    "_Adjusted R²_", round(cr_age_lme$r2, 3)
  ),
  ...
  " " = c(
    "_Adjusted R²_", round(bm_age_lme$r2, 3)
  )
)
```

__Table 5__: _Linear Mixed Effects Models with Interactions between Age and Resilience, CR, or BM on Memory_
```{r tab5, echo=FALSE} 
# Format and print the table
...
# Fit the mixed-effects model
model_res_lme <- lmer(memory ~ age * resilience + (1 | id), data = long_data_vis_res)

# Define a range of ages for prediction
age_seq <- seq(min(long_data_vis_res$age), max(long_data_vis_res$age), length.out = 100)

# Generate predictions for each resilience group
predictions <- do.call(rbind, lapply(unique(long_data_vis_res$resilience), function(res) {
  new_data <- data.frame(
    age = age_seq,
    resilience = res,
    id = 1
...
# Define colors for each resilience group
palette <- c('High Resilience' = '#1f77b4', 'Low Resilience' = '#ff7f0e')

# Create the plot
resilience_plot <- ggplot() +
  ...
  geom_ribbon(data = predictions, aes(x = age, ymin = lower_bound, ymax = upper_bound, fill = as.factor(resilience)), alpha = 0.3) +  # Add CIs
  geom_line(data = predictions, aes(x = age, y = predicted, color = as.factor(resilience)), size = 1.2) + # Plot predicted lines for each group
...
```
```{r echo=FALSE, message=FALSE, warning=FALSE}
# Trying the GAMM model instead of the LME

# Fit a GAMM model using gamm4 with interaction
# gamm4_model <- gamm4(memory ~ s(age, by = resilience, k = 24) + factor(resilience), 
#                      random = ~(1|id), data = long_data_vis_res)
# 
# 
# # Create a new data frame for predictions
# prediction_data <- expand.grid(
#   age = seq(min(long_data_vis_res$age), max(long_data_vis_res$age), length.out = 100),
#   resilience = levels(long_data_vis_res$resilience)
# )
# prediction_data$id <- NA  # Random effect placeholder
# 
# # Generate predictions using the GAMM model
# prediction_data$predicted <- predict(gamm4_model$gam, newdata = prediction_data, type = "response")
# 
# # Create the plot
# resilience_plot <- ggplot() +
#   geom_line(data = long_data_vis_res, aes(x = age, y = memory, group = id), color = "lightgray", alpha = 0.5) +
#   geom_point(data = long_data_vis_res, aes(x = age, y = memory), color = "gray", size = 1) +
#   geom_ribbon(data = predictions, aes(x = age, ymin = lower_bound, ymax = upper_bound, fill = as.factor(resilience)), alpha = 0.3) +
#   geom_line(data = prediction_data, aes(x = age, y = predicted, color = as.factor(resilience)), size = 1.2) +
#   scale_color_manual(values = palette) +
...
get_gamm_metrics <- function(k, df, response_var, lme_mod) {
  ...
  if (max_k > 1) {
    ...
    mod_em <- gamm4(as.formula(paste(response_var, "~ s(age, k =", max_k, ") +", make.names("study"))),
                    data = df, random = ~(1|id)) # Assuming 'id' is your random effect variable
...
# Uncomment this code to check GAMM assumptions
# gamm_memory <- gamm4(memory ~ 1 + s(age, k = 12) + study, data = long_clean_data, random = ~(1|id))
# gam.check(gamm_memory$gam)
# 
# gamm_hc <- gamm4(adj_HIP_total ~ 1 + s(age, k = 12) + study, data = long_clean_data, random = ~(1|id))
...
gamm_table <- data.frame(
  ...
  " " = c(
    "_LRT p-value_",
    format_p_value(gamm_compare_em_memory$p_value_lrt),
    " "
  ),
  ...
  " " = c(
    "_LRT p-value_",
    format_p_value(gamm_compare_em_adj_hc$p_value_lrt),
    " "
  )
)
```
...
We initially fit the GAMM using a range of basis functions and compared their fit to the linear model. Despite variations in the GAMM fit, the conclusions regarding how the GAMM and the linear compared did not change. In Supplementary Table 1, we fit the GAMMs with four basis functions as increasing the number beyond this point did not substantially change the AIC nor BIC values. The smooth age-term was significant in the memory GAMM (effective degrees of freedom (EDF) = `r round(gamm_compare_em_memory$EDF, 3)`, _p_-value = `r format_p_value(gamm_compare_em_memory$p_value_gamm)` and the hippocampus GAMM (EDF = `r round(gamm_compare_em_adj_hc$EDF, 3)`, _p_-value = `r format_p_value(gamm_compare_em_adj_hc$p_value_gamm)`. For the linear models, both models had significant age coefficients (memory: β = `r round(mem_age_lme$coef, 3)`, _p_ = `r format(format_p_value(mem_age_lme$p_value), scientific = FALSE)`; adjusted hippocampal volume β = `r round(hc_age_lme$coef, 3)`, _p_ = `r format(format_p_value(hc_age_lme$p_value), scientific = FALSE)`). For memory, the linear and GAMM fits were similar; however, the linear fit was better according to BIC and LRT. For adjusted hippocampal volume, the GAMM fit was better according to AIC and LRT.
...
```
<small>(A) The association between memory and age with individual trajectories and a generalized additive model (EDF = `r round(gamm_compare_em_memory$EDF, 3)`) and confidence intervals. (B) The association between adjusted hippocampal volume and age with individual trajectories with a generalized additive model (EDF = `r round(gamm_compare_em_adj_hc$EDF, 3)`) and confidence intervals.</small>
...
memory_hippocampus_models <- list(
...
# CR and BM Analyses
cr_bm_models <- list(
  cr_validation, bm_age_lme, cr_age_lme, resilience_lme
)

# Functional Connectivity Analyses Longitudinal
...
# Functional Connectivity Analyses Cross-sectional
...
model_comp_general_models <- list(
  gamm_compare_em_memory, gamm_compare_em_adj_hc,
  hc_age_lme, mem_age_lme
)

# Define a function to extract p-values
extract_p_value <- function(model) {
  # Check if the model contains p-value information and return it
...
memory_hippocampus_p_values <- adjust_p_values(memory_hippocampus_models, c("emhp_merged", "model_baseline", "model_followup"))
education_p_values <- adjust_p_values(education_models, c("edu_baseline", "edu_followup", "edu_long", "edu_cr", "edu_bm", "edu_cr_int", "edu_bm_int"))
cr_bm_p_values <- adjust_p_values(cr_bm_models, c("cr_validation", "bm_age_lme", "cr_age_lme", "resilience_lme"))
