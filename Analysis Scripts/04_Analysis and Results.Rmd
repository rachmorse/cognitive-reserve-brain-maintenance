...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# 1) Fit the lm model
lm_mem_hc_study <- lm(memory_slopes ~ hc_slopes + study,
               data = clean_data)

# 2) Generate predicted values 
pred_data <- ggpredict(
  lm_mem_hc_study, 
  terms = "hc_slopes[all]"  # The predictor for the x-axis
)

# 3) Plot partial effect of hc_slopes on memory_slopes
mem_hc_plot <- ggplot() +
  geom_point(
    data = clean_data, 
  ...
  # Confidence ribbon from ggpredict
  geom_ribbon(
    data = pred_data, 
...
cr_path_plot <- ggplot(cr_path, aes(pathway, reverse_cr, color = reverse_cr, fill = reverse_cr)) +
  ...
  scale_y_continuous(
    limits = c(0.5, 2.7), # Replace with your desired limits
    expand = c(0, 0),
...
# 1) Fit a model for sn_slopes, controlling for study and grouping by reverse_cr_group
fit_sn <- lm(memory_slopes ~ sn_slopes * reverse_cr_group + study,
             data = clean_data)

# 2) Use ggpredict() to get the partial‐effect data
#    "reverse_cr_group" is used as a by‐variable to get separate lines
pred_sn <- ggpredict(fit_sn, terms = c("sn_slopes[all]", "reverse_cr_group"))

# 3) Plot memory_slopes plus the partial‐effect lines
sn_int <- ggplot() +
...
# 4) Repeat the same steps for ec_slopes:
fit_ec <- lm(memory_slopes ~ ec_slopes * reverse_cr_group + study,
             data = clean_data)

pred_ec <- ggpredict(fit_ec, terms = c("ec_slopes[all]", "reverse_cr_group"))
