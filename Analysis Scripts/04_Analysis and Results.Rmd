...
clean_data <- clean_data %>%
  mutate(
    ...
    closer_to = ifelse(dist_to_cr_line > dist_to_bm_line, "BM", "CR")
  )
...
We then created the CR and BM index with participants classified as on the CR pathway (N = `r cr_pathway`/`r total_participants`) or the BM pathway (N = `r bm_pathway`/`r total_participants`) (Figure 4). The mean BM value for participants on the BM pathway was `r round(mean_bm, 3)` (range = `r round(min_bm, 3)` - `r round(max_bm, 3)`), and the mean CR value for participants on the CR pathway was `r round(mean_cr, 3)` (range = `r round(min_cr, 3)` - `r round(max_cr, 3)`).

```{r echo=FALSE, warning=FALSE}
clean_data <- clean_data %>%
  mutate(
    total_cr_bm_test = ifelse(reverse_bm > reverse_cr, reverse_bm, reverse_cr),
    pathway = ifelse(reverse_bm > reverse_cr, "BM Pathway", "CR Pathway"),
  )
...
# Plot the CR half
cr_path_plot <- ggplot(cr_path, aes(pathway, total_cr_bm_test, color = reverse_cr, fill = reverse_cr)) +
  ...
  scale_y_continuous(
    limits = c(0.7, 2.7), # Replace with your desired limits
    expand = c(0, 0),
    position = "right" # Move y-axis to the right
  ) +
  labs(
    y = "Amount of Cognitive Reserve",
    x = "Participants on Cognitive Reserve Path",
    color = "Cognitive Reserve"
  ) +
  guides(color = "none", fill = guide_colorbar(title = "Cognitive Reserve"))

# Plot the BM half
bm_path_plot <- ggplot(bm_path, aes(pathway, total_cr_bm_test, color = reverse_bm, fill = reverse_bm)) +
  geom_quasirandom(size = 4) +
  scale_color_gradientn(
  ...
  labs(
    y = "Amount of Brain Maintenance",
    x = "Participants on Brain Maintenance Path",
    color = "Brain Maintenance"
  ) +
  theme(
    axis.text.x = element_blank(), # Remove x-axis text
...
```{r fig3, echo=FALSE, dpi = 400, fig.width=8, fig.height=6, message=FALSE, warning=FALSE}
index_plot
```

An index depicting participants as either on the BM or CR pathway. The y-axis represents the total CR for those on the CR path or the total BM for those of the BM path.
...
test_bm <- lm(memory_slopes ~ hc_slopes + study + reverse_bm, data = bm_data)
summary(test_bm)

test_bm_mmse <- lm(mmse ~ hc_slopes + study + reverse_bm, data = bm_data)
summary(test_bm_mmse)

test_cr <- lm(memory_slopes ~ hc_slopes + study + reverse_cr, data = cr_data)
summary(test_cr)

filt_df_test <- clean_data %>%
  filter(em_obs > 2)
View(filt_df_test)


left_data <- clean_data %>%
  filter(closer_to == "CR")

right_data <- clean_data %>%
  filter(closer_to == "BM")

# Plot for left side
left_plot <- ggplot(left_data, aes(x = dist_cr_or_bm, y = 0, color = reverse_cr, fill = reverse_cr)) +
  geom_quasirandom(size = 6) +
  scale_color_gradientn(colors = c("#f5d7b0", "#f5be27", "#fa8d00", "#bc507f", "#7E1891", "#1c0159")) +
  scale_fill_gradientn(colors = c("#f5d7b0", "#f5be27", "#fa8d00", "#bc507f", "#7E1891", "#1c0159")) +
  theme_minimal() +
...
# Plot for right side
right_plot <- ggplot(right_data, aes(x = dist_cr_or_bm, y = 0, color = reverse_bm, fill = reverse_bm)) +
  geom_quasirandom(size = 6) +
  scale_color_gradientn(colors = c("#E5E483", "#8ac926", "#4CCD99", "#5BBCFF", "#024CAA", "#00224D")) +
  scale_fill_gradientn(colors = c("#E5E483", "#8ac926", "#4CCD99", "#5BBCFF", "#024CAA", "#00224D")) +
  theme_minimal() +
  theme(
...
combined_plot <- left_plot + right_plot + plot_layout(ncol = 2, widths = c(1, 1))
combined_plot
```
