...
jn_plot <- jn_plot + 
  ...
  labs(caption = "A)", title = "Johnson-Neyman Plot") +
...
# Fit the mixed-effects model
model_lme_cr_sn <- lmer(sn ~ reverse_cr + (1 | id) + study + time, long_cr_pathway_fc)

reverse_cr_seq <- seq(
  from = min(long_cr_pathway_fc$reverse_cr, na.rm = TRUE),
  to   = max(long_cr_pathway_fc$reverse_cr, na.rm = TRUE),
  length.out = 100
)
...
em_2group <- emmeans(
  model_lme_cr_sn,
  specs = ~ reverse_cr,
  at    = list(reverse_cr = reverse_cr_seq),
  reff  = 0
)
...
predictions <- summary(em_2group) %>%
  ...
  mutate(reverse_cr = as.numeric(reverse_cr))

# Create the plot
p2 <- ggplot() +
  geom_point(data = long_cr_pathway_fc, aes(x = reverse_cr, y = sn), color = "grey", size = 1, alpha = 0.8) + 
  geom_ribbon(data = predictions, aes(x = reverse_cr, ymin = lower_bound, ymax = upper_bound, ), fill = "#3caebf", alpha = 0.3) + 
  geom_line(data = predictions, aes(x = reverse_cr, y = predicted), color = "#3caebf", size = 1.2) +  
  labs(
    x = "Cognitive Reserve Variable", 
    y = "Anterior Salience Network Connectivty", 
    color = "Group", 
    fill = "Group", 
  ...
  theme(legend.position = "bottom",
        ...
        plot.margin = margin(t = 55, l = 15), # Increase left and top margin
        plot.title = element_text(size = 11)) 

cr_fc <- jn_plot + 
  xlab("Anterior Salience Network Connectivity") +
  ylab("Effect of Hippocampal Volume on Memory") + p2 + plot_layout(ncol = 2) 
```

__Figure 5:__ _The Relationships Between Cognitive Reserve, Functional Connectivity, and Memory_

```{r fig5, echo=FALSE, dpi = 400, fig.width=7, fig.height=5, message=FALSE, warning=FALSE}
cr_fc
```
A) A Johnson-Neyman plot of how the effect of hippocampal volume on memory changes as anterior Salience Network connectivity changes. The plot shows that anterior Salience Network connectivity decreases the relationship between hippocampal volume and memory and that the relationship between hippocampal volume and memory is only significant when Salience Network connectivity is less than 0.14. B) The association between our cognitive reserve variable and anterior Salience Network connectivity plotted using a linear mixed effects model that controls for cohort/MRI scanner using estimated marginal means. The model produces a coefficient of `r format(round(cr_sn$coef, 3), scientific = FALSE)` (scaled: `r round(cr_sn_unscaled$coef, 3)`) with confidence intervals. The lower panel highlights the anterior Salience Network as defined in the Shirer atlas. Abbreviations: ECN Executive Control Network, SN Salience Network, CR Cognitive Reserve, n.s. not significant.
...
__Table 5__: _Models Examining Education and Functional Connectivity as Moderators of Hippocampal Volume and Memory_
```{r tab5, echo=FALSE} 
# Format and print the table
...
Note. Models are adjusted for age, sex, education, follow-up time, and cohort/MRI scanner. _p_-values are adjusted for multiple comparisons using the FDR. Abbreviations: DMN Default Mode Network, ECN Executive Control Network, SN Salience Network, CI Confidence Interval.
...
__Table 6__: _Models Examining our Cognitive Reserve measures and Functional Connectivity and Education_
```{r tab6, echo=FALSE} 
# Format and print the table
kable(long_fc, format = "markdown", col.names = c(" ", "**Cognitive Reserve**", " ", " ", " ", "**Cognitive Reserve**", " ", " "))
```
