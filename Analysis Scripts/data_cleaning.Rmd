...
# Set the timepoint to 1 for all participants
...
waha_df_struct_controls$Visual_Memory_ROCF_Imm <- as.numeric(waha_df_struct_controls$Visual_Memory_ROCF_Imm)
...
cobra_df <- merge(cobra_struct, cobra_func, by = c("id", "timepoint"), all = TRUE)
```
...
# Recode sex. Currently its coded opposite
betula_em$sex <- recode(betula_em$sex, "2" = "male", "1" = "female")

# Merge Betula dfs into one
betula_df <- merge(betula_em, betula_df, by = c("id", "timepoint"), all = TRUE)
```
...
# Subset oslo_struct to only include necessary data
columns_to_keep <- c(
  "timepoint",
  "id",
  "age",
...
# Drop participants with scanner ousPrisma and drop participants without MRI
oslo_struct <- oslo_struct %>%
...
oslo_func2 <- merge(oslo_func_scanners, oslo_func, by = c("id", "timepoint"), all = TRUE)

# Merge Oslo dfs into one
oslo_df <- merge(oslo_struct, oslo_func2, by = c("id", "timepoint", "scanner"), all = TRUE)
```
...
# For each person keep only the scanner with the most timepoints
scanner_keep <- oslo_df %>%
  filter(!is.na(scanner)) %>%
  count(id, scanner, name = "n") %>%
  group_by(id) %>%
  arrange(desc(n), desc(scanner == "ousSkyra")) %>% # prioritize ousSkyra 3T
  slice(1) %>%
  ungroup() %>%
  select(id, scanner)

# Keep only rows from the selected (id, scanner) pairs
oslo_df <- oslo_df %>%
  semi_join(scanner_keep, by = c("id", "scanner"))
...
scanner_cols <- paste0("scanner.", 1:6)
oslo_df_scanners <- select(
  oslo_df,
  id, scanner_cols
)

# Check that participants are not being dropped
df_multi_timepoint <- oslo_df_scanners %>%
  mutate(n_timepoints = rowSums(!is.na(pick(all_of(scanner_cols))))) %>%
...
determine_scanner_type <- function(scanners) {
  if ("ousAvanto" %in% scanners) {
    return("oslo_avanto")
  } else if ("ousSkyra" %in% scanners) {
    return("oslo_skyra")
  } else {
    return(NA_character_) # Return NA if no scanner
  }
}
...
oslo_test <- select(oslo_df, id, study, paste0("scanner.", 1:6))
```
...
# Filter the min age to be 60 years
merged_cohorts <- merged_cohorts %>%
  filter(age_baseline >= 60)
...
# Create a new variable for study
merged_cohorts <- merged_cohorts %>%
...
cleaned_data <- merged_cohorts %>%
  group_by(cohort) %>% # Group by cohort so that 3 SD is calculated separately for each
  mutate(across(
    .cols = setdiff(names(merged_cohorts), c("cohort", exclude_cols)),
