...
# Plot the CR half
cr_path_plot <- ggplot(cr_pathway, aes(pathway, reverse_cr, color = reverse_cr, fill = reverse_cr)) +
...
# Plot the BM half
bm_path_plot <- ggplot(bm_pathway, aes(pathway, reverse_bm, color = reverse_bm, fill = reverse_bm)) +
  geom_quasirandom(size = 4, alpha = 0.7) +
  scale_color_gradientn(
...
# Then edu and memory
edu_mem <- get_mixed_effects_stats(scale(memory) ~ scale(edu) + (1|id) + study + age_baseline + mem_time + sex, data = long_clean_data_edu_mem_hc, vars)

# Then is edu related to our CR variable
edu_cr <- get_regression_stats(scale(edu) ~ scale(reverse_cr) + study, data = cr_pathway, int_vars)
```

__**Cognitive Reserve, Education, and Functional Connectivity:**__

Longitudinal mixed-effects analyses provided no evidence that education moderated the relationship between hippocampal volume and memory (_p_ = `r format_p_value(edu_cr_int$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_cr_int", "adjusted_p_value"])`), nor was there a significant relationship between our CR variable and education (_p_ = `r format_p_value(edu_cr$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_cr", "adjusted_p_value"])`) in participants aging CR-like. However, education was significantly correlated with memory (β = `r round(edu_mem$coef, 3)`, _p_ = `r format(format_p_value(edu_mem$p_value), scientific = FALSE)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_mem", "adjusted_p_value"])`, marginal r² = `r round(edu_mem$marginal_r2, 3)`, conditional r² = `r round(edu_mem$conditional_r2, 3)`; Table 5). 
...
```{r fig5, echo=FALSE, dpi = 400, fig.width=5, fig.height=5, message=FALSE, warning=FALSE}
cr_fc
```
Johnson-Neyman plot of how the hippocampal volume-memory relationship changes as anterior Salience Network connectivity changes. The plot shows that as anterior Salience Network connectivity decreases, the hippocampal volume-memory relationship increases, only becoming significant when Salience Network connectivity is less than 0.09SD above the mean. Above that threshold, the hippocampal volume–memory relationship is not significant. Abbreviations: SN Salience Network, n.s. not significant. 
...
long_fc <- data.frame(
  ...
  "**Education * Hippocampal Volume**" = c(
    ...
    paste0(round(edu_mem$coef, 3), " (", round(edu_mem$ci[1], 3), " to ", round(edu_mem$ci[2], 3), ")"),
    "**Education**", "_Coefficient (95% CI)_",
  ...
  " " = c(
    "_Marginal R²_", round(edu_cr_int$marginal_r2, 3),
    "", "_Marginal R²_", round(edu_mem$marginal_r2, 3),
  ...
  "**DMN Connectivity * Hippocampal Volume**" = c(
    ...
    paste0(round(cr_dmn_mem$coef, 3), " (", round(cr_dmn_mem$ci[1], 3), " to ", round(cr_dmn_mem$ci[2], 3), ")"),
    "**DMN Connectivity**", "_Coefficient (95% CI)_",
  ...
  " " = c(
    ...
    "", "_Marginal R²_", round(cr_dmn_mem$marginal_r2, 3),
  ...
  "**ECN Connectivity * Hippocampal Volume**" = c(
    ...
    paste0(round(cr_ec_mem$coef, 3), " (", round(cr_ec_mem$ci[1], 3), " to ", round(cr_ec_mem$ci[2], 3), ")"),
    "**ECN Connectivity**", "_Coefficient (95% CI)_",
  ...
  " " = c(
    ...
    "", "_Marginal R²_", round(cr_ec_mem$marginal_r2, 3),
  ...
  "**Anterior SN Connectivity * Hippocampal Volume**" = c(
    ...
    paste0(round(cr_sn_mem$coef, 3), " (", round(cr_sn_mem$ci[1], 3), " to ", round(cr_sn_mem$ci[2], 3), ")"),
    "**Anterior SN Connectivity**", "_Coefficient (95% CI)_",
  ...
  " " = c(
    ...
    "", "_Marginal R²_", round(cr_sn_mem$marginal_r2, 3),
...
education_cr_models <- list(edu_cr, edu_cr_int, edu_mem)

# CR and BM Sensitivity Analyses
cr_sensitivity_models <- list(cr_validation)
...
raw_barplot_by_cohort <- function(df, values, cohort = cohort, id = NULL,
                                  title = NULL, ylab = "Raw score", ncol = 1) {
  cohort <- enquo(cohort)
  id_quo <- enquo(id)
  value_names <- tidyselect::eval_select(enquo(values), df) |> names()

  # Build an ID if one isn't provided
  d <- df
  if (quo_is_null(id_quo) || as_label(id_quo) == "NULL") {
    d <- d %>% group_by(!!cohort) %>% mutate(.id = row_number()) %>% ungroup()
  } else {
    d <- d %>% mutate(.id = !!id_quo)
  }

  # Long format; order bars by cohort so colors group together
  long <- d %>%
    select(all_of(value_names), !!cohort, .id) %>%
    pivot_longer(all_of(value_names), names_to = "measure", values_to = "value") %>%
    filter(!is.na(value)) %>%
    group_by(measure) %>%
    arrange(!!cohort, .id, .by_group = TRUE) %>%
    mutate(.x = factor(row_number())) %>%
    ungroup()

  # Where to draw separators between cohorts (per facet)
  sep <- long %>%
    count(measure, !!cohort, name = "n") %>%
    group_by(measure) %>%
    mutate(cum = cumsum(n)) %>%
    filter(row_number() < n()) %>%
    ungroup()

  p <- ggplot(long, aes(x = .x, y = value, fill = !!cohort)) +
    geom_col(width = 0.9) +
    facet_wrap(~ measure, ncol = ncol, scales = "free_y") +
    labs(title = title, x = NULL, y = ylab, fill = "Cohort") +
    theme_minimal(base_size = 12) +
    theme(
      legend.position = "top",
      axis.text.x = element_blank(),       # hide cluttered IDs
  ...
  if (nrow(sep) > 0) {
    p <- p + geom_vline(
      data = sep,
      aes(xintercept = cum + 0.5),
      linetype = "dashed", linewidth = 0.3, inherit.aes = FALSE
    )
  }
...
# 1) Raw bars for memory_slopes (one bar per participant, colored by cohort)
p1 <- raw_barplot_by_cohort(
  clean_data,
  values = memory_slopes,
  cohort = cohort,          # change if your cohort col is named differently
  id     = id,      # optional; omit if you don't have an ID column
  title  = "Raw memory_slopes by participant (colored by cohort)"
)
p1

# 2) Raw bars for memory_1 and memory_2 (two facets, same cohort colors)
p2 <- raw_barplot_by_cohort(
  clean_data,
  values = c(memory_baseline, memory_followup),
  cohort = cohort,
  id     = id,      # optional
  title  = "Raw Memory at tp1 and tp2 by participant",
  ylab   = "Raw memory score",
  ncol   = 1                # stack facets vertically; use 2 for side-by-side
)
p2
```

Supplementary Figure on cohort distribution of episodic memory scores.
```{r}
