...
format_p_value <- function(p) {
  if (p < 0.0001) {
    return("<0.0001") # Written like this because of HTML formatting
  } else {
    return(round(p, 4))
  }
}
...
emhp_merged <- get_regression_stats(scale(memory_slopes) ~ scale(hc_slopes) + study + scale(age_baseline) + scale(FU_time) + sex + scale(edu), data = clean_data, hc_vars)
emhp_merged_unscaled <- get_regression_stats(memory_slopes ~ hc_slopes + study + age_baseline + FU_time + sex + edu, data = clean_data, hc_vars)
...
model_followup <- get_regression_stats(scale(memory_followup) ~ scale(adj_HIP_total_followup) + study + scale(age_followup) + scale(edu), data = clean_data_followup, hc_vars)
```

Hippocampus and memory annual change were significantly correlated (β = `r round(emhp_merged$coef, 4)`, _p_ = `r format_p_value(emhp_merged$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "emhp_merged", "adjusted_p_value"])`, r² = `r round(emhp_merged$r2, 4)`) (Figure 2). Cross-sectionally, hippocampal volume and memory were not significantly correlated at baseline (_p_ = `r format_p_value(model_baseline$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "model_baseline", "adjusted_p_value"])`) but were significantly correlated at follow-up (β = `r round(model_followup$coef, 4)`, _p_ = `r format_p_value(model_followup$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "model_followup", "adjusted_p_value"])`, r² = `r round(model_followup$r2, 4)`). These results are summarized in Table 4. 

```{r echo=FALSE, message=FALSE, warning=FALSE}
# 1) Fit the lm model
lm_mem_hc_study <- lm(memory_slopes ~ hc_slopes + study + age_baseline + FU_time + sex + edu, data = clean_data)
...
clean_data <- clean_data %>%
  mutate(
    high_cr = ifelse(CR > 0.5, "High CR", "Low CR")
  )
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# Run LME for sensitivity analysis 
...
edu_cr_int <- get_mixed_effects_stats(scale(memory) ~ scale(edu) * scale(adj_HIP_total) + (1|id) + (1|study) + age + FU_time + sex, data = long_clean_data, vars)

# Then edu and memory
edu_mem <- get_mixed_effects_stats(scale(memory) ~ scale(edu) + (1|id) + (1|study) + age + FU_time + sex, data = long_clean_data, vars)
...
# Check then if explains mem above and beyond HC
edu_cr_hc <- get_mixed_effects_stats(scale(memory) ~ scale(edu) + scale(adj_HIP_total) + (1|id) + (1|study) + age + FU_time + sex, data = long_clean_data, vars)
```

__**Cognitive Reserve, Education, and Functional Connectivity:**__

Longitudinal mixed-effects analyses provided no evidence that education moderated the relationship between hippocampal volume and memory (_p_ = `r format_p_value(edu_cr_int$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_cr_int", "adjusted_p_value"])`), nor was there a significant relationship between our CR variable and education (_p_ = `r format_p_value(edu_cr$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_cr", "adjusted_p_value"])`) in high CR participants. However, education was significantly correlated with memory (β = `r round(edu_mem$coef, 4)`, _p_ = `r format(format_p_value(edu_mem$p_value), scientific = FALSE)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_mem", "adjusted_p_value"])`, marginal r² = `r round(edu_mem$marginal_r2, 4)`, conditional r² = `r round(edu_mem$conditional_r2, 4)`), and the correlation remained significant when hippocampal change was included in the model (β = `r round(edu_cr_hc$coef, 4)`, _p_ = `r format(format_p_value(edu_cr_hc$p_value), scientific = FALSE)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "edu_cr_hc", "adjusted_p_value"])`, marginal r² = `r round(edu_cr_hc$marginal_r2, 4)`, conditional r² = `r round(edu_cr_hc$conditional_r2, 4)`; Table 5). 
...
priority_var_cr <- c(
  ...
  "scale(dmn_baseline):scale(adj_HIP_total_baseline)",
  "scale(dmn_followup):scale(adj_HIP_total_followup)",
  "scale(ec_baseline):scale(adj_HIP_total_baseline)",
  "scale(ec_followup):scale(adj_HIP_total_followup)",
  "scale(sn_baseline):scale(adj_HIP_total_baseline)",
  "scale(sn_followup):scale(adj_HIP_total_followup)"
)

cr_dmn_stats <- get_mixed_effects_stats(scale(memory) ~ scale(dmn) * scale(adj_HIP_total) + (1|id) + (1|study) + age + FU_time + sex + edu, data = long_clean_data, priority_var_cr)
cr_ecn_stats <- get_mixed_effects_stats(scale(memory) ~ scale(ec) * scale(adj_HIP_total) + (1|id) + (1|study) + age + FU_time + sex + edu, data = long_clean_data, priority_var_cr)
cr_sn_stats <- get_mixed_effects_stats(scale(memory) ~ scale(sn) * scale(adj_HIP_total) + (1|id) + (1|study) + age + FU_time + sex + edu, data = long_clean_data, priority_var_cr)
```
...
cr_dmn <- get_mixed_effects_stats(scale(dmn) ~ scale(CR) + (1|id) + (1|study) + age + FU_time + sex + edu, data = long_cr_pathway_fc, priority_var_int_cr)
cr_ec <- get_mixed_effects_stats(scale(ec) ~ scale(CR) + (1|id) + (1|study) + age + FU_time + sex + edu, data = long_cr_pathway_fc, priority_var_int_cr)
cr_sn <- get_mixed_effects_stats(scale(sn) ~ scale(CR) + (1|id) + (1|study) + age + FU_time + sex + edu, data = long_cr_pathway_fc, priority_var_int_cr)

cr_dmn_mem <- get_mixed_effects_stats(scale(memory) ~ scale(dmn) + (1|id) + (1|study) + age + FU_time + sex + edu, data = long_clean_data, priority_var_int_cr)
cr_ec_mem <- get_mixed_effects_stats(scale(memory) ~ scale(ec) + (1|id) + (1|study) + age + FU_time + sex + edu, data = long_clean_data, priority_var_int_cr)
cr_sn_mem <- get_mixed_effects_stats(scale(memory) ~ scale(sn) + (1|id) + (1|study) + age + FU_time + sex + edu, data = long_clean_data, priority_var_int_cr)
```


With functional connectivity, the longitudinal mixed-effects analyses revealed that anterior SN connectivity moderated the relationship between hippocampal volume and memory (β = `r round(cr_sn_stats$coef, 4)`, _p_ = `r format(format_p_value(cr_sn_stats$p_value), scientific = FALSE)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_sn_stats", "adjusted_p_value"])`, marginal r² = `r round(cr_sn_stats$marginal_r2, 4)`, conditional r² = `r round(cr_sn_stats$conditional_r2, 4)`), such that participants with higher anterior SN connectivity did not have a significant relationship between memory and hippocampal volume. Anterior SN connectivity was also positively correlated with memory (β = `r round(cr_sn_mem$coef, 4)`, _p_ = `r format(format_p_value(cr_sn_mem$p_value), scientific = FALSE)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_sn_mem", "adjusted_p_value"])`, marginal r² = `r round(cr_sn_mem$marginal_r2, 4)`, conditional r² = `r round(cr_sn_mem$conditional_r2, 4)`), and there was a positive relationship between our CR variable and anterior SN connectivity in high CR participants (β = `r round(cr_sn$coef, 4)`, _p_ = `r format(format_p_value(cr_sn$p_value), scientific = FALSE)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_sn", "adjusted_p_value"])`, marginal r² = `r round(cr_sn$marginal_r2, 4)`, conditional r² = `r round(cr_sn$conditional_r2, 4)`). There was also evidence that DMN connectivity moderated the hippocampal volume-memory relationship (β = `r round(cr_dmn_stats$coef, 4)`, _p_ = `r format_p_value(cr_dmn_stats$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_dmn_stats", "adjusted_p_value"])`, marginal r² = `r round(cr_dmn_stats$marginal_r2, 4)`, conditional r² = `r round(cr_dmn_stats$conditional_r2, 4)`); however, unlike with anterior SN connectivity, when DMN connectivity is high, there is a significant relationship between hippocampal and memory change, such that decreases in hippocampal volume are associated with increases in memory. The moderation analyses are summarized in Figure 5 which depicts how the hippocampal volume-memory relationship increases as anterior SN connectivity decreases, demonstrating a significant positive relationship between memory and hippocampal volume only for participants with anterior SN connectivity less than -0.02SD above the mean. Figure 5 also depicts how the hippocampal volume-memory relationship increases as DMN connectivity decreases; however, in this case, it demonstrates a significant positive relationship between memory and hippocampal volume for participants with DMN connectivity less than 0.08SD above the mean and for participants with DMN connectivity greater than 3.7SD above the mean. This indicates that, among participants with high DMN connectivity, there is a negative hippocampal-memory relationship. Additionally with DMN connectivity, there was no evidence of a relationship between DMN connectivity and memory (_p_ = `r format_p_value(cr_dmn_mem$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_dmn_mem", "adjusted_p_value"])`), or of a relationship between our CR variable and DMN connectivity in high CR participants (_p_ = `r format_p_value(cr_dmn$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_dmn", "adjusted_p_value"])`).

With ECN connectivity, there was no evidence that it moderated the relationship between hippocampal volume and memory (_p_ = `r format_p_value(cr_ecn_stats$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_ecn_stats", "adjusted_p_value"])`). There was also no evidence of a relationship between ECN connectivity and memory (_p_ = `r format_p_value(cr_ec_mem$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_ec_mem", "adjusted_p_value"])`), or of a relationship between our CR variable and ECN connectivity in high CR participants (_p_ = `r format_p_value(cr_ec$p_value)`, _p_<sub>FDR</sub> = `r format_p_value(p_value_df[p_value_df$model_name == "cr_ec", "adjusted_p_value"])`). All functional connectivity results are summarized in Table 5. Figure 6 shows the difference in anterior SN connectivity when comparing low versus high CR groups. 

```{r echo=FALSE, message=FALSE, warning=FALSE}
# Fit the model 
model <- lmer(scale(memory) ~ scale(sn) * scale(adj_HIP_total) + (1|id) + 
              study + age + FU_time + sex + edu, 
...
jn_plot <- jn_plot + 
  ...
  theme(plot.caption = element_text(size = 10, hjust = -0.2),
        ...
        plot.tag.position = c(1, 0.01),  # bottom-left
        title = NULL,
...
jn_plot_dmn <- jn_result_dmn$plot +
  ...
  theme(
    legend.position = "bottom",
    plot.tag.position = c(0, 0.01),  # bottom-left
...
jn_plot_dmn$layers <- jn_plot_dmn$layers[-1]
...
cr_fc <- cr_fc + cr_fc_dmn + plot_layout(ncol = 2, guides = "collect") &
  theme(legend.position = "bottom")
```

__Figure 5:__ _Anterior Salience Network Functional Connectivity and the Hippocampal-Memory Relationship_

```{r fig5, echo=FALSE, dpi = 400, fig.width=7, fig.height=5, message=FALSE, warning=FALSE}
cr_fc
```
A) Johnson-Neyman plot of how the hippocampal volume-memory relationship changes as anterior Salience Network connectivity changes. The plot shows that as anterior Salience Network connectivity decreases, the hippocampal volume-memory relationship increases, only becoming significant when Salience Network connectivity is less than -0.02SD above the mean. Above that threshold, the hippocampal volume–memory relationship is not significant. B) The hippocampal volume-memory this time depicted with Default Mode Network connectivity changes. The plot shows that as Default Mode Network connectivity decreases, the hippocampal volume-memory relationship increases. However, this hippocampal-memory relationship is significant for both high and low Default Mode Network connectivity values, only becoming non-significant when Default Mode Network connectivity is between 0.08-3.70SD above the mean. Within that range, the hippocampal volume–memory relationship is not significant. Abbreviations: SN Salience Network, DMN Default Mode Network, n.s. not significant. 

__Figure 6:__ _Cognitive Reserve and Anterior Salience Network Connectivity_

Connectivity differences (high CR vs low CR) within the anterior SN among participants classified as aging CR-like. For each region, connectivity was averaged across sessions, scaled, adjusted for scanner/cohort effects, and expressed as the difference between the high CR and low CR groups; all values are positive, indicating stronger connectivity in the high CR group in all regions. Abbreviations: CR, cognitive reserve; SN, Salience Network.    
...
long_fc <- data.frame(
  ...
  "**Education * Hippocampal Volume**" = c(
    "_Coefficient (95% CI)_",
    ...
    "**Education**", "_Coefficient (95% CI)_",
    paste0(round(edu_cr$coef, 4), " (", round(edu_cr$ci[1], 4), " to ", round(edu_cr$ci[2], 4), ")")
  ),
  " " = c(
...
```{r tab5, echo=FALSE} 
# Format and print the table
kable(long_fc, format = "markdown", col.names = c(" ", "**Education * Hippocampal Volume**", " ", " ", "**DMN Connectivity * Hippocampal Volume**", " ", " ", "**ECN Connectivity * Hippocampal Volume**", " ", " ", "**Anterior SN Connectivity * Hippocampal Volume**", " ", " "))
```
...
Supplementary Figure 1 depicts distributions of normalized memory composite scores by cohort across timepoints. The box-and-jitter plots show the distribution of harmonized memory scores across cohorts following the normalization process. The resulting distributions overlap across cohorts, with means centered near zero and standard deviations close to one, indicating that the harmonization procedure places all cohorts on a common scale. 

**_Cohort Distributions of Episodic Memory Scores_**
```{r echo=FALSE, message=FALSE, warning=FALSE}
...
plot_memory_all_tps <- function(df, cohort, tps) {
  ...
  # Compute mean and SD for annotation
  mean_labels <- long_tp %>%
  ...
  p <- ggplot(long_tp, aes(x = !!cohort, y = score, fill = !!cohort)) +
    ...
    labs(x = NULL, y = "Memory Composite", fill = "Cohort") +
...
Note. Abbreviations: M mean, SD standard deviation, n number.
...
mod_lme_mem <- lmer(memory ~ age + (1|id), data = long_clean_data)
aic_lm_mem <- AIC(mod_lme_mem)
bic_lm_mem <- BIC(mod_lme_mem)

mod_lme_hc <- lmer(adj_HIP_total ~ age + (1|id), data = long_clean_data)
...
suppressMessages({
  mem_age_lme <- get_mixed_effects_stats(scale(memory) ~ scale(age) + (1|id), data = long_clean_data, age)
  hc_age_lme <- get_mixed_effects_stats(scale(adj_HIP_total) ~ scale(age) + (1|id), data = long_clean_data, age)
})
```
...
# for (k in 1:27) {
#   model_mem <- gamm4(memory ~ s(age, k = k+3), data = long_clean_data, random = ~(1|id))
# 
...
# # Loop through k values and print results
# for (k in 1:27) {
#   model_hc <- gamm4(adj_HIP_total ~ s(age, k = k+3), data = long_clean_data, random = ~(1|id))
# 
#   cat("Model with k =", k + 3, "\n")
...
# }
# 
# gamm_memory <- gamm4(memory ~ s(age, k = 4), data = long_clean_data, random = ~(1|id))
# gam.check(gamm_memory$gam)
# 
# gamm_hc <- gamm4(adj_HIP_total ~ s(age, k = 5), data = long_clean_data, random = ~(1|id))
...
get_gamm_metrics <- function(k, df, response_var, lme_mod) {
  ...
  if (max_k > 1) {
    ...
    mod_em <- gamm4(as.formula(paste(response_var, "~ 1 + s(age, k =", max_k, ")")),
      data = df, random = ~ (1 | id), REML   = FALSE
    )
...
mem_age_unscaled <- get_mixed_effects_stats((memory) ~ age + (1 | id), data = long_clean_data, vars_unscale)
```
<small>A) The association between memory and age with individual trajectories and a generalized additive mixed model (EDF = `r round(gamm_compare_em_memory$EDF, 4)`) and confidence intervals. B) The association between hippocampal volume and age with individual trajectories with a generalized additive mixed model (EDF = `r round(gamm_compare_em_adj_hc$EDF, 4)`) and confidence intervals. Abbreviations: EDF effective degrees of freedom.</small>
...
education_cr_models <- list(edu_cr, edu_cr_int, edu_mem, edu_cr_hc)
...
education_cr_p_values <- adjust_p_values(
  education_cr_models,
  c("edu_cr", "edu_cr_int", "edu_mem", "edu_cr_hc")
)
